...
`Enumeration.reads` are plain lists a caller in `machinome.motion` fills
...
class Enumeration:
    ...
    `machinome.motion.couplings`'s business (a `_Deferred` bundle of a
...
def warn_legacy_render(assembly, read):
    ...
    ``__main__`` and test runners; a maker running ``machinome build`` has to
