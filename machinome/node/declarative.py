...
module holds the first. `machinome.parameters` holds the second, and
...
from machinome.parameters import (_RESERVED, Declaration, ParameterError,
...
# `machinome.motion.ports` imports `machinome.node.phase`, and
# `machinome.motion.joints` imports ports, so this module -- which the
...
class ChildDeclaration:
    ...
    def __getattr__(self, attribute):
        ...
        from machinome.motion.couplings import (BroadcastRef, PathRef,
                                                  read_through)
...
class RepeatDeclaration:
    ...
    def __getattr__(self, attribute):
        ...
        (`machinome.motion.couplings.BroadcastRef`) rather than an
        ordinary path -- usable as the DRIVEN end of a relation and
        refused as its source."""
        if attribute.startswith('_'):
            raise AttributeError(attribute)
        from machinome.motion.couplings import BroadcastRef, read_through
...
_RELATIONS_KEY = '__machinome_relations__'
...
# REQUEST -- `machinome/simulation/clocked.py` -- because a class body
...
def _validate_controls(cls, name, table):
    ...
    `machinome/node/` importing nothing from `machinome/simulation/`,
