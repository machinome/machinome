...
# as `machinome.node.base.trimesh`, which is the target
...
def __getattr__(name):
    ...
    562), so reading `machinome.node.base.trimesh` works in a process
...
def _compose_matrix(node, stop_before):
    ...
    perturbation assertions in machinome/test.py insert an operation
...
# (machinome.node.phase) says which one. An operation applied through
...
class AbstractBaseNode(metaclass=NodeMeta):
    """A mechanical project in machinome is represented by a
    ...
    # `FlexibleNode` sets it (see machinome/node/flexible.py).
    flexible = False
    ...
    def __init__(self, *args, name=None, **kwargs):
        ...
        # machinome.motion.joints imports machinome.motion.ports,
        ...
        # reason the joints import is: machinome.motion.couplings
        # imports machinome.motion.ports, which imports this package.
        from machinome.motion.couplings import (
