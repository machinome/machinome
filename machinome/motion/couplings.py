...
Module scope imports `machinome.motion.ports` and nothing else: a
...
from machinome.motion.ports import (CLOCK_NAME, BoundPort, Port,
                                     ...
                                     wiring_binding)
from machinome.node.phase import current as _current_phase
from machinome.node.phase import current_enumeration as _current_enumeration
...
def _state_declaration(value):
    ...
    """
    from machinome.node.qualified import StateDeclaration
...
class StateRef(DriverRef):
    ...
    def check_declared_on(self, owner, relation):
        from machinome.node.qualified import declared_states_of
...
def _named_in_body(value):
    ...
    """
    from machinome.node.declarative import executing_body
...
def _coefficient(ref, other, symbol):
    ...
    if _is_number(other):
        return other
    from machinome.parameters import Expression
...
def commit(sources, targets, at=None, law=None, **rejected):
    ...
    """
    from machinome.node.declarative import record_commitment
...
class ResolvedEnd:
    ...
    def __init__(self, node, declared, ref):
        from machinome.motion.joints import Joint, declared_joints
...
def relate(driver, driven, ratio=None, offset=None, law=None):
    ...
    body that is executing."""
    from machinome.node.declarative import record_relation
