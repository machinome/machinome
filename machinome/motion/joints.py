...
(`machinome.node.declarative.ChildDeclaration`).
...
Module scope imports `machinome.motion.ports` and nothing else: a joint
...
from machinome.motion.ports import (BoundPort, Coordinate, Port,
...
class Joint(Coordinate):
    ...
    def __init__(self, axis, at=(0, 0, 0), range=None, unit=None):
        ...
        # `machinome.node.declarative.ChildDeclaration.__init__` the
    ...
    def _refuse_self_read(self, owner, ref, side):
        from machinome.motion.couplings import OwnRef
    ...
    def place(self, node, value):
        ...
        """
        from machinome.node.base import apply_joint_motion
    ...
    def _carry(self, node, axes, points):
        ...
        hand (`machinome.motion.couplings` never touches this: a joint
...
def re_place_declared_joints(node):
    ...
    restore` (`machinome.node.assembly`) uses it to undo a refused
    `set_state`, and the test runner's checkpoint restore
    (`machinome.manager.test`) uses it to make a restored child's
