...
"""Sphinx extension embedding `machinome export` output in documentation.

Add ``'machinome.sphinx'`` to ``extensions`` in conf.py, then::

    .. machinome:: path/to/export_dir
       :height: 300px
       :t: 0.25
       :autoplay: no

The argument is a directory produced by ``machinome export`` (relative to
...
installed machinome-viewer package's widget files, so committed exports only need to
...
from machinome import __version__
from machinome.viewers import bundle as viewer_bundle

logger = logging.getLogger(__name__)

# Kept in sync with machinome.core.export.MANIFEST_FORMAT (asserted
# by tests); duplicated so the extension imports without the CAD stack
MANIFEST_FORMAT = 'machinome-export'

# Directory under the HTML output where exports are copied
OUTPUT_DIR = '_machinome'

WIDGET_FILES = ('index.html', 'machinome-viewer.js')
...
class machinome_iframe(nodes.General, nodes.Element):
    pass


class Machinome(SphinxDirective):
    """Embeds a `machinome export` directory as an <iframe>."""
    ...
    def run(self):
        ...
        if not os.path.isdir(abs_path):
            raise self.error(
                f'machinome: no export directory at "{rel_path}". '
                'Generate it with: machinome export <node.py> -o '
                f'{rel_path}'
            )
        ...
        if not os.path.isfile(manifest_path):
            raise self.error(
                f'machinome: "{rel_path}" has no manifest.json -- '
                'not a `machinome export` output directory'
            )
        try:
        ...
        if not isinstance(manifest, dict) \
           or manifest.get('format') != MANIFEST_FORMAT:
            raise self.error(
                f'machinome: {rel_path}/manifest.json is not a '
        ...
        exports = getattr(self.env, 'machinome_exports', None)
        if exports is None:
            exports = self.env.machinome_exports = {}
        info = exports.setdefault(
            dest, {'path': abs_path, 'docnames': set()},
        )
        if info['path'] != abs_path:
            raise self.error(
                f'machinome: exports "{info["path"]}" and '
    ...
    def warn_unreadable(self, rel_path, manifest):
        ...
        if version in rendered:
            return
        logger.warning(
            f'machinome: the export "{rel_path}" declares document '
...
def visit_machinome_iframe(self, node):
...
def copy_exports(app):
    ...
    exports = getattr(app.env, 'machinome_exports', {})
...
def _complete_widget(target):
    ...
    if not viewer_bundle.has_bundle():
        ...
        logger.warning(
            f'machinome: {viewer_bundle.missing_bundle_remedy()}'
        )
        return
    sources = {
        'index.html': viewer_bundle.index_path(),
        'machinome-viewer.js': viewer_bundle.bundle_path(),
    }
    for name in missing:
...
def purge_exports(app, env, docname):
    exports = getattr(env, 'machinome_exports', {})
...
def merge_exports(app, env, docnames, other):
    exports = getattr(env, 'machinome_exports', None)
    if exports is None:
        exports = env.machinome_exports = {}
    for dest, info in getattr(other, 'machinome_exports', {}).items():
