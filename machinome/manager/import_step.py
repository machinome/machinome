...
def _part_class_source(class_name, step_source, part_name, part_index):
    ...
    # blank line before the comment block byte-for-byte when it is not.
    index_line = (f"    part_index = {part_index}\n"
                 if part_index is not None else "")
    return (
...
def generate_parts(assembly, step_file, into_dir):
    """`(source_text, {product_identity: class_name})` for `parts.py`:
    one `StepNode` subclass per PRODUCT that is a part (spec "Generated
    part source"), never for a sub-assembly or the root, and never
    ...
    for product in assembly.products:
        if product.name is not None:
            name_counts[product.name] = name_counts.get(product.name, 0) + 1
    ...
    for product in assembly.products:
        ...
        class_names[product.identity] = class_name
        shared = name_counts.get(product.name, 0) > 1
        bodies.append(_part_class_source(
            class_name, step_relative, product.name,
            product.part_index if shared else None))

    header = (
        f"# Machinome - generated by `machinome import-step`\n\n"
        f'"""Parts scaffolded from {os.path.basename(step_file)!r} by '
        f'`machinome import-step`.\n\n'
...
def _ordered_assembly_identities(assembly):
    ...
    sub-assemblies sharing one name are two entries here, each with its
    own children, never one merged bucket."""
    assembly_identities = {product.identity for product in assembly.products
                          if product.kind in ('sub-assembly', 'root assembly')}
    children_by_parent = {}
    for occurrence in assembly.occurrences:
        if occurrence.product_identity in assembly_identities:
            children_by_parent.setdefault(
                occurrence.parent_identity, []).append(
                    occurrence.product_identity)
...
def _attribute_names(occurrences):
    ...
    Stays keyed on the product NAME, not its identity (design D3): the
    question this suffix answers is "does this parent place this name
    more than once", which is the readable one for a human attribute --
    keying it on identity would turn one parent holding two DIFFERENT
    `Pin` products into `pin` and `pin_2` instead of `pin_1` and
    `pin_2`. Do not "fix" this to identity."""
    counts = {}
...
def _child_expression(occurrence, class_names, assembly_class_names):
    if occurrence.product_identity in class_names:
        return f'{class_names[occurrence.product_identity]}()'
    return f'{assembly_class_names[occurrence.product_identity]}()'


def _placement_lines(attribute, occurrence, step_basename):
    """The placement comment and, when the occurrence is not the
    identity, its `rotate`/`translate` statements -- and whether any
    statement was emitted at all (design D4): `_assembly_class_source`
    needs that to know whether the class it is building needs a `pass`
    of its own."""
...
def _assembly_class_source(class_name, occurrences, attributes, class_names,
    ...
    any_emitted = False
    for attribute, occurrence in zip(attributes, occurrences):
        placement_lines, emitted = _placement_lines(
            attribute, occurrence, step_basename)
        render_lines.extend(placement_lines)
        any_emitted = any_emitted or emitted
    if not render_lines:
        # No child at all: the pre-existing childless-assembly fallback.
        render_lines = ['        pass']
    elif not any_emitted:
        # Every child is a comment-only identity placement: the method
        # body would otherwise be comments alone, which does not parse
        # (design D4, evidence.md §4).
        render_lines.append('        pass')
...
def generate_assembly(assembly, model_name, step_file, class_names):
    ...
    products_by_identity = {product.identity: product
                            for product in assembly.products}
    order = _ordered_assembly_identities(assembly)  # always ends with None

    occurrences_by_parent = {}
    for occurrence in assembly.occurrences:
        occurrences_by_parent.setdefault(
            occurrence.parent_identity, []).append(occurrence)
    ...
    for identity in order:
        if identity is None:
            ...
            name = products_by_identity[identity].name
            assembly_class_names[identity] = _unique(
                _class_name_base(name), seen)
    root_class_name = assembly_class_names[None]

    bodies = []
    for identity in order:
        occurrences = occurrences_by_parent.get(identity, [])
        attributes = _attribute_names(occurrences)
        bodies.append(_assembly_class_source(
            assembly_class_names[identity], occurrences, attributes,
...
class ImportStep:
    ...
    def handle(self, args):
        ...
        print()
        print('[tool.machinome.models]')
        print(f'{model_name} = "{reference}"')
        print()
        print('Next steps:')
        print(f'  machinome build {model_name}')
        print(f'  machinome develop {model_name}')
