...
class Test:
    """Nodes may implement tests by inheriting machinome.test.TestCaseMixin
    ...
    def select_all(self, path):
        ...
        if not project.named:
            self.fail(f'{project.manifest} declares no models; --all walks '
                      f'a [tool.machinome.models] table')
        selections = []
    ...
    def resolve_path(self, path):
        """Users and agents routinely hand `machinome test` the TEST file
        ...
        `machinome test` has an unambiguous reason to do this; `develop`
    ...
    def restore_children_checkpoints(self, node):
        ...
        from machinome.motion.joints import re_place_declared_joints
