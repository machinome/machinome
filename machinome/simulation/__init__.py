...
The dependency runs one way. machinome.node never imports this
...
for the same reason `machinome/node/__init__.py` resolves its backend
...
def __getattr__(name):
    ...
    if module_name is None:
        ...
        # `machinome.simulation.sim` after importing only the package.
