...
entry to it), while `machinome.node` imports nothing from here -- a
node that declares no driver never pulls the simulation layer in.
"""

from dataclasses import dataclass

from machinome.node.qualified import DriverDeclaration, declared_drivers_of
...
class Driver(DriverDeclaration):
    ...
    stays here. `machinome.node` imports nothing from this package.
