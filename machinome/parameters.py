...
`machinome.node.declarative`, which imports this module: the children a
class declares, and the metaclass that makes a class body recognizable.
`machinome.motion.ports` holds the third, and `DriverDeclaration` in
`machinome.node.qualified` the runtime one; all four follow the same
...
def function_formula(name, numeric, *args):
    """A formula applying one of `machinome.math`'s functions to
...
def declared_parameters(node_class):
    ...
    `machinome.node.declarative` answers the same question about the
