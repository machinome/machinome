...
def _warn_unreadable(version):
    ...
    `machinome develop` serves it and the browser refuses it BY NAME, which is
...
def anchor_build_dir(build_root, build_dir):
    ...
    process -- `machinome test --all` walking the models -- starts from the
...
class Builder(FileSystemEventHandler):
    ...
    def __init__(self, path, is_reload=False, build_dir=None,
        ...
        # `machinome develop` invocation keeps the old, non-surviving
