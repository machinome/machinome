...
"""`machinome.motion` -- what moves, and what drives what.
...
and the declared time base here from `machinome.node`, verbatim, and
leaves `joints` and `couplings` empty for the two cycles that follow.

The move is deliberately unshimmed: `machinome.node` stops answering for
...
class MotionPortsExportsTest(BaseNodeTest):
    """`machinome.motion.ports` is the one home for ports and the time
...
class MotionSubmodulesTest(TestCase):
    ...
    def test_the_couplings_module_names_the_relation(self):
        import machinome.motion.couplings as couplings
...
class MotionPackageExportsNothingTest(TestCase):
    ...
    def test_the_package_itself_exports_no_name(self):
        import machinome.motion as motion
...
class OldPathRefusedTest(TestCase):
    """`machinome.node` no longer answers for a port or the time base --
        ...
        for name in self.MOVED:
            with self.subTest(name=name):
                with self.assertRaises(ImportError) as raised:
                    exec(f'from machinome.node import {name}\n', {})
    ...
    def test_reading_the_removed_submodules_gives_the_same_redirect(self):
        ...
        # `from machinome.node import RotationalPort` (above) carry this
...
class ImportOrderTest(TestCase):
    ...
    def test_node_internal_imported_first(self):
        result = probe('import machinome.node.internal\n' +
                       PORT_SNIPPET + "print('DONE')\n")
        self.assertEqual(result.stdout.strip(), 'DONE', result.stderr)


class MotionImportCostTest(TestCase):
    """`machinome.motion` and its submodules stay as cheap to import as
    `machinome.node` -- no CAD backend, no exact-geometry stack."""

    def test_the_empty_package_pulls_in_no_other_machinome_module(self):
        """The package itself is still free. Neither submodule is:
        cycle 2 filled `joints` and cycle 3 filled `couplings`, and both
        cost what `machinome.motion.ports` costs, because a joint owns
    ...
    def test_ports_pulls_no_cad_backend(self):
        result = probe('import machinome.motion.ports\n' + "print('DONE')\n")
        self.assertEqual(result.stdout.strip(), 'DONE', result.stderr)
        self.assertFalse(result.imported('cadquery'),
                         'importing machinome.motion.ports imported cadquery')
        self.assertFalse(result.imported('OCP'),
                         'importing machinome.motion.ports imported the '
...
class RunningEngineImportCostTest(TestCase):
    ...
    The compile step and the engine live under `machinome.simulation`
    and are imported by `Sim.__init__` only when the root declares
    `Time.running()`; the running time base and the run-binder marker
    live in `machinome.motion.ports`, which imports nothing new for
    them.
    """
    ...
    def test_a_looping_roots_simulation_imports_no_running_engine(self):
        result = probe(
            ...
            "print('DONE')\n").check()
        self.assertEqual(result.stdout.strip(), 'DONE', result.stderr)
        self.assertFalse(result.imported('machinome.simulation.run'),
                         'a looping simulation imported the running engine')
        self.assertFalse(result.imported('machinome.simulation.program'),
                         'a looping simulation imported the compile step')

    def test_ports_and_couplings_import_nothing_from_the_simulation_layer(self):
        for module in ('machinome.motion.ports', 'machinome.motion.couplings'):
            with self.subTest(module=module):
                result = probe(f'import {module}\n' + "print('DONE')\n").check()
                self.assertFalse(result.imported('machinome.simulation'),
    ...
    def test_a_running_roots_simulation_imports_the_engine(self):
        result = probe(
            ...
            "print('DONE')\n").check()
        self.assertEqual(result.stdout.strip(), 'DONE', result.stderr)
        self.assertTrue(result.imported('machinome.simulation.run'))
        self.assertTrue(result.imported('machinome.simulation.program'))
        self.assertFalse(result.imported('cadquery'))
        self.assertFalse(result.imported('OCP'))
