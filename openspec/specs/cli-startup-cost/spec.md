...
run. Importing `machinome.cli` SHALL NOT import any command implementation
...
- **WHEN** `machinome.cli` is imported in a fresh interpreter
- **THEN** no `machinome.manager.*` command implementation module is present
  in `sys.modules`

#### Scenario: One command does not load another

- **WHEN** `machinome viewer` runs to completion
...
The system SHALL keep `machinome -h` and the no-subcommand invocation listing every
...
The system SHALL answer `machinome viewer` from the installed viewer package's
...
- **WHEN** `machinome viewer` runs against an installation with `machinome-viewer`
...
- **WHEN** `machinome viewer` runs in an installation without `machinome-viewer`
...
STL, not at `machinome.node.base` module scope. Importing
`machinome.node.base` SHALL NOT import `trimesh`.

The system SHALL also keep `trimesh` out of `machinome.node.base` by way of
`machinome.node.operations`, which imports it at its own module scope.
...
`machinome.node.base`, so a caller that patches it keeps working whether or
not a mesh has yet been read.

#### Scenario: Importing the node base loads no mesh library

- **WHEN** `machinome.node.base` is imported in a fresh interpreter
...
importing it SHALL import no other `machinome` module, no CAD backend and
...
- **THEN** no other `machinome` module and no CAD backend appear among the
  imported modules
