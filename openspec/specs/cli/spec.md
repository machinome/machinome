...
The `machinome` command-line interface: command-first grammar, environment
...
Code: `machinome/cli.py`, `machinome/manager/{develop,test,snapshot,new,
export}.py`.
## Requirements
...
environment variables always win. Recognized variables: `MACHINOME_PORT`
(backend, default 8000), `MACHINOME_FRONTEND_PORT` (npm dev server, default
3000), `SOLID_BUILD_DIR` (default `_build`).

#### Scenario: Worktree ports

- **WHEN** a worktree's `.env` sets `MACHINOME_PORT=8003`
- **THEN** `machinome develop` run from that directory serves on port 8003,
...
The system SHALL provide `machinome develop [reference]`, orchestrating the
...
`machinome-viewer` package and fail before starting development processes,
naming `pip install "machinome[viewer]"`, when that package is not installed.
...
package or bind `MACHINOME_PORT`. The browser viewer SHALL run as the installed
...
- **WHEN** `machinome develop` runs the browser viewer
- **THEN** the viewer is the installed viewer package's `serve` command,
  launched through the interpreter running `machinome` on the project's build
  directory, and it is restarted after each rebuild cycle

### Requirement: Build command

The system SHALL provide `machinome build [reference]` as a node-scoped command
...
`machinome build --all` SHALL build every declared model in declaration order,
...
- **WHEN** a user runs `machinome build` successfully
...
- **WHEN** a user runs `machinome build windmill.windmill:NoSuchClass`
...
- **WHEN** a user runs `machinome build --all` in a project declaring
...
- **WHEN** `machinome build --all` runs and `wall_clock_01` fails to assemble
...
- **WHEN** `machinome build --all` runs and `wall_clock_01`'s module raises on
...
- **WHEN** a user runs `machinome build --all` in a project that declares no
  models
- **THEN** the command exits nonzero saying the project declares no models

### Requirement: Callback mode validation

The `machinome develop` command SHALL accept `--callback URL` in normal browser
mode and in `--no-web` mode, and reject it with `--web-dev` with a clear
argument error. `machinome build` SHALL NOT accept a callback option.

#### Scenario: Callback requested for viewer-development mode

- **WHEN** a user runs `machinome develop --web-dev --callback URL`
- **THEN** the command exits with a clear argument error before starting
  development processes

#### Scenario: Callback for an external viewer host

- **WHEN** a user runs `machinome develop --no-web --callback URL`
...
`machinome snapshot --time` SHALL keep its meaning as a 0.0–1.0 position on the
...
- **WHEN** `machinome snapshot --time 0.5` runs on a project whose root declares
...
- **WHEN** `machinome snapshot` runs on a running root with no `--time`
...
The system SHALL provide `machinome export [reference]` with `-o/--output` (default
...
- **WHEN** a user runs `machinome export -o docs/_exports/windmill`
- **THEN** the output directory contains `manifest.json`, `models/`, and the
  embeddable widget files

### Requirement: Viewer command

The system SHALL provide `machinome viewer`, a command that takes no node path and
...
`pip install "machinome[viewer]"` on standard error, and exit 1.
...
- **WHEN** a program runs `machinome viewer` against an installation with
  `machinome-viewer`
...
- **WHEN** a user runs `machinome viewer` in an installation without
  `machinome-viewer`
...
- **WHEN** `machinome build engine.py --set bore=32.0` runs on a root declaring
  `bore = Length(30.0)`
...
- **WHEN** `machinome develop piston.py` loads a leaf declaring
  `diameter = Length()` without `--set diameter=...`
- **THEN** the command fails naming the class and the parameter, and
  `machinome develop piston.py --set diameter=29.4` loads it
...
The system SHALL provide `machinome models`, a command that takes no node reference
...
`machinome viewer`. A malformed manifest SHALL be reported on standard error with
...
- **WHEN** a user runs `machinome models` in a project declaring `wall_clock_01`
...
- **WHEN** a program runs `machinome models --json` in a project declaring models
...
- **WHEN** a user runs `machinome models --json` in a project declaring only
...
- **THEN** `machinome models` reports that model's state as `failed`
...
`machinome snapshot --renderer openscad` SHALL fail when OpenSCAD reports that it
...
- **WHEN** `machinome snapshot` renders a `.scad` whose `import()` names a file
...
- **WHEN** `machinome snapshot` renders a design whose imports all resolve
