...
- [x] 1.1 Record the byte size and sha256 of `tests/clocked-corpus.json`,
...
- [x] 1.2 Record the probe of `Request`'s fields over the `Calculator`
...
- [x] 2.1 `tests/clocked_project/unsupported.py`: two declarations beside the
...
- [x] 2.2 RED: `tests/test_clocked_refusals.py` asserts that `Sim(TwoInputs())`
  and `Sim(NoInput())` are refused naming the instruction, the inputs it names
  and the one-input rule. Fails: both construct today.
- [x] 2.3 GREEN: the arity refusal in `compile_clocked`'s instruction loop
  ...
  driver.
- [x] 2.4 REGRESSION: the existing State-target refusal still fires with its
  own message over `Instructed`, unabsorbed by the new one.

## 3. `trigger` is a request (design §1, §3, §4, §6)

- [x] 3.1 RED: `tests/test_clocked_sim.py` asserts that over the `Calculator`
...
- [x] 3.2 RED: the same for `sim.trigger('Set four')` against
  `sim.move('operand', to=4)` — the `targets=` branch, over an `int` driver.
- [x] 3.3 RED: two roots differing ONLY in the declared `duration` (2.0 and
  0.0) make equal requests and equal banks: the machine reads no duration
  (§4).
- [x] 3.4 RED: `sim.trigger('nothing')` under a clocked root raises `KeyError`
...
- [x] 3.5 GREEN: `Sim.trigger` (`solid_node/simulation/sim.py:514`) drops
...
- [x] 3.6 GREEN: `_not_clocked`'s message stops implying `trigger` is refused
  and names the request verbs a clocked caller has.
- [x] 3.7 REGRESSION: the cadence refusal test loses `trigger` and keeps
...
- [x] 4.1 `tests/clocked_project/calculator.py`: `instructions = {'Stroke':
...
- [x] 4.2 RED: `tests/test_clocked_sim.py` asserts that a `by=` request from a
...
- [x] 4.3 RED: a CLIPPED request reports `end` at the stop's landing and
...
- [x] 4.4 GREEN: `Request.__slots__` and `__init__` gain `origin` and
...
- [x] 4.5 The docstrings of `Request` and `Clocked.move` say what the two ends
  are, in which units, and why they are not left for a caller to recompute.

## 5. The corpus records a trigger (design §8)

- [x] 5.1 RED: `tests/test_clocked_corpus.py` asserts that the committed
  ...
  and no such fields.
- [x] 5.2 RED: the generator's inventory test asserts that a corpus playing no
...
- [x] 5.3 GREEN: `tools/generate_clocked_corpus.py` — a `{'trigger': '<name>'}`
...
- [x] 5.4 Regenerate `tests/clocked-corpus.json` and
...
- [x] 5.5 The replay reproduces the file exactly, `tolerance.float` still
  `0.0`, and a `trigger` step's recorded fields equal those of the same
  request made by hand in the same script.
- [x] 5.6 The corpus's own document copy for `Calculator` differs from task
  1.1's only in its `instructions` table — asserted, not inspected.

## 6. The document does not change (design §7)

- [x] 6.1 REGRESSION, not red-first: `tests/test_clocked_document.py` asserts
...
- [x] 6.2 RED: publishing a root whose instruction names two drivers writes no
  document and is refused by the machine's compile, through a build and
  through an export. Fails: it publishes today.
- [x] 6.3 GREEN: nothing in `solid_node/core/serializer.py` changes. If this
  task needs a code change, the design is wrong and the pilot decides.

## 7. Zero behaviour change elsewhere (design §13)

- [x] 7.1 `tests/running-corpus.json` replays unchanged and is byte-identical
  to task 1.1's record.
- [x] 7.2 A running root's `trigger` still returns its tuple of commands,
  ...
  returns `None`.
- [x] 7.3 A stateless model's document is byte-identical, and the clocked
  counter still reads ZERO across its construction, render, symbolic walk,
  compile and `document_body`.
- [x] 7.4 `git diff --stat` shows no change under `solid_node/simulation/run.py`,
  `solid_node/simulation/program.py` or `solid_node/core/`.

## 8. Documentation and measurement

- [x] 8.1 `docs/scenarios.rst`: `trigger` leaves the "What a clocked model
...
- [x] 8.2 `docs/api-reference.rst`: `Request`'s two new fields;
  `HISTORY.rst`: one entry naming the originating project.
- [x] 8.3 Measure `trigger` against the same `move` over the `Calculator`,
  median of 20, and record both in `evidence.md`: the difference must be a
  dictionary lookup.
- [x] 8.4 Record in `evidence.md` every number this cycle measured, the red log
  for every task above, and anything found that contradicts `design.md`
  rather than silently resolving it.

## 9. Close the record

- [x] 9.1 `workflow/warts.md`: mark "the instruction's meaning under a clocked
...
- [x] 9.2 ADR-129 (NODE) extracted after implementation per design §12, the
...
- [x] 9.3 Sync the baseline specs, archive the change, and run
  `openspec validate --strict`.
- [x] 9.4 Report what the viewer's cycle inherits: a returned `Request` with
