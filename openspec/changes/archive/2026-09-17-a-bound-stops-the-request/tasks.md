...
- [x] 1.1 `tests/clocked_project/pawl.py`: cycle 1's `Counter` with a
...
- [x] 1.2 `tests/clocked_project/lock.py`: the note's interlock sketch AS
...
- [x] 1.3 `tests/clocked_project/freeze.py`: the same machine with the pair
  of bounds that read the knob's own committed value, gated on the crank's
  phase (design §4). This is the acceptance fixture.
- [x] 1.4 `tests/clocked_project/bounds_unsupported.py`: a bounded
...
- [x] 1.5 `tests/clocked_project/gate.py`: the STATE-reading fixture of
...
- [x] 1.6 `tests/clocked_project/decorative.py` (a ranged joint NOTHING
...
- [x] 2.1 RED: a test asserting that constructing `Sim` over the PAWL
...
- [x] 2.2 GREEN: `compile_bounds` in `solid_node/simulation/clocked.py` —
...
- [x] 2.3 RED: the message tests for each row of design §3's refusal table,
  asserting the joint, the node, the side and the offending thing are named.
- [x] 2.4 GREEN: those refusals, at construction, beside cycle 1's.
- [x] 2.5 Give `_compiled_spans` and `_qualified_reads` the NAME of the
...
- [x] 3.1 RED: affine, kinked, jumped and curved levels classified per
  driver; a level no driver moves classified `constant` and not examined.
- [x] 3.2 GREEN: `_shape_of` over the level with every name but the moving
...
- [x] 3.3 RED: a jumped level's path partitioned at its own surfaces and
...
- [x] 3.4 GREEN: `JumpPlan.cuts` consumed, no second partition written.

## 4. The clip (design §4, §6, §7)

- [x] 4.1 RED: the level's own-coordinate argument read ONCE, at the
...
- [x] 4.2 GREEN: the level assembled per request — own coordinate as a
  number from the chain at `t = 0`, reads through their chains, standing
  drivers and states as numbers.
- [x] 4.3 RED: the threshold `max(0, g(0))` on the OUTSIDE fixture — a
  ...
  nothing.
- [x] 4.4 GREEN: the direction test as one threshold.
- [x] 4.5 RED: the landing — a bound met exactly at a representable value
...
- [x] 4.6 GREEN: the ordinal walk to the SATISFIED side, through
  `far_side_of`'s bisection with membership decided by EVALUATING the level.

## 5. The request (design §9, §11)

- [x] 5.1 RED: the clip precedes the events — a request whose full travel
...
- [x] 5.2 GREEN: the truncation of `delta` at the head of `Clocked.move`,
  with cycle 1's event loop untouched.
- [x] 5.3 RED: a request stopped at zero travel is ADMITTED — no exception,
  nothing committed, the bank identical entry for entry, one stop reported.
- [x] 5.4 GREEN: the zero-travel path.
- [x] 5.5 RED: `Request.admitted` in design units on a scaled driver;
  `Request.stops` in the documented shape; two constraints at ONE landing
  reported as two entries.
- [x] 5.6 GREEN: the `Stop` value object and the two `Request` fields.
- [x] 5.7 RED: `sim.stops` bounded under `record=N`, absent without it, and
  no longer refused by name.
- [x] 5.8 GREEN: the ring in `sim.py` and the removal of `stops` from the
  clocked refusal list.

## 6. One authority (design §10)

- [x] 6.1 RED: for each fixture, at twenty fractions of a request path, the
...
- [x] 6.2 GREEN: whatever the composition needs to make that true; a failure
  here is a composition that simplified.
- [x] 6.3 RED: during a REQUEST the enumeration does not judge a compiled
...
- [x] 6.4 GREEN: the MARK — `(node, joint)` identities held for the duration
...
- [x] 6.5 RED: a request whose COMMIT carries a compiled coordinate out of
...
- [x] 6.6 GREEN: the end-of-request judgement over the final bank, through
...
- [x] 7.1 RED: the FREEZE fixture — the knob free at rest, admitting ZERO
  travel off rest, and the crank running its whole travel with the knob set.
- [x] 7.2 RED: the LOCK fixture (the note's sketch) stopping the crank at
  phase 1 with the knob set, asserted in the same test, so the correction of
  design §4 is evidence and not an opinion.
- [x] 7.3 RED: a plain numeric range (`range=(0, 9)`) stopping a request of
  twenty at exactly `9`, inclusive, with the pose accepting it.
- [x] 7.4 RED: the GATE — one `move('crank', by=1000)` admitting 300 against
...
- [x] 7.5 RED: the DECORATIVE fixture — a ranged joint nothing binds
...
- [x] 7.6 RED: the OUTSIDE fixture — the direction test of §7 on a level that
...
- [x] 8.1 The whole existing suite green.
- [x] 8.2 A running fixture's stop — including one whose bound reads another
  coordinate — unchanged in behaviour, records and messages.
- [x] 8.3 The pin tumbler lock's close-of-enumeration judgement unchanged,
  ...
  request's pose.
- [x] 8.4 Cycle 1's register fixture, whose joints declare no range: every
  request admits its whole travel, reports no stop, and gives the same
  events and the same bank as before.
- [x] 8.5 A byte-identical document for an existing published fixture, and
  no clocked path entered by a stateless tree.

## 9. Measurement, records and the ADR

- [x] 9.1 Take design §18's measurements on the fixtures only and record
  them in `evidence.md`, compared to nothing.
- [x] 9.2 `docs/scenarios.rst` and `HISTORY.rst`.
- [x] 9.3 After implementation and tests confirm the final design, extract
  ONE NODE ADR, candidate **ADR-126**, per design §14, and update the ADR
  index. Do not write it before.
  Written as `docs/adrs/NODE/ADR-126-a-bound-stops-a-clocked-request-on-its-path.md`,
  indexed after ADR-125 in `docs/adrs/README.md`, with `docs/architecture.md`'s
  clocked-mode synthesis and Known gaps rewritten to match.
- [x] 9.4 Record in `workflow/warts.md` anything the implementation found
