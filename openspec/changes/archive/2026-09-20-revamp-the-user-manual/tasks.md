...
- [x] 1.1 Replace the docs tests that pin the old chapters and the three
...
- [x] 1.2 Add `tests/test_tutorial_counter.py`, which imports, assembles and
...
- [x] 2.1 Create `docs/tutorial/` (manifest, `counter/` package, digit artwork
...
- [x] 2.2 Export the chapters that embed a model into `docs/_exports/` and
  commit them; remove the retired tutorial exports.

## 3. The pages

- [x] 3.1 Write the entry pages: why, install, first machine, index.
- [x] 3.2 Write the ten tutorial chapters against the chapter modules.
- [x] 3.3 Write the how-to guides from the existing leaf, fusion, markings,
  flexible, declaring, animation, CLI and testing material.
- [x] 3.4 Write the concept pages from the existing declaring, driving,
  scenarios, node-tree, testing and embedding material.
- [x] 3.5 Write the examples index and three example pages; remove the V8 and
  Clock 01 pages and submodules; add the Pascaline and Curta submodules;
  update `.readthedocs.yaml` and the Actions docs job.
- [x] 3.6 Write the reference and project pages: CLI, API, Sphinx directive,
...
- [x] 4.1 Strict Sphinx build (`-E -W --keep-going`) with all three examples
...
- [x] 4.2 Record the evidence in `validation.md`, sync the spec delta,
