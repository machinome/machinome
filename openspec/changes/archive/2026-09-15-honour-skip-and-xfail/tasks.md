...
- [x] 1.1 Add `SkipAndExpectedFailureTest` to `tests/test_manager_test.py`,
...
- [x] 1.2 Add to the same file the summary-and-exit assertions:
...
- [x] 1.3 Run it and record which assertions are red and why:
  `PYTHONPATH="$PWD" /home/asa/devel/libresolid-studio/.venv/bin/python -m pytest tests/test_manager_test.py -x -q`.

## 2. Make it green

- [x] 2.1 `manager/test.py`: add `num_skipped`, `num_expected_failures`,
  `num_unexpected_successes` to `__init__` (`:48-52`).
- [x] 2.1a `run_test`: wrap the `setUp` call so that `unittest.SkipTest`
...
- [x] 2.2 `run_test` (`:325-383`): add an `except unittest.SkipTest` arm
...
- [x] 2.3 `run_test`: compute the verdict from `step_pass`/`step_fail`/
...
- [x] 2.4 `run_test`: the failfast break at `:369` fires only on a REAL
  ...
  UNEXPECTED SUCCESS.
- [x] 2.5 `run_class_tests` (`:311-323`): before `setUpClass`, honour
...
- [x] 2.6 `report` (`:285-300`): append `, S skipped`, `, X expected
...
- [x] 2.7 `handle` (`:179-180`): exit 1 when `num_failed` or
  `num_unexpected_successes`. `run_tests` (`:302-308`) still does not exit.
- [x] 2.8 Re-run `tests/test_manager_test.py` green.

## 3. Prove nothing else moved

- [x] 3.1 `tests/test_manager_test.py`, `tests/test_named_models.py`,
...
- [x] 3.2 The whole suite once before the implementation commit, with its
  summary line and exit code recorded in `evidence.md`.
- [x] 3.3 Re-run `evidence/run-probes.sh` and record the new transcripts
  ...
  exit 1.
- [x] 3.4 Remove `evidence/skip_xfail_project/_build/` and every
  `__pycache__/` the probes created under the fixture before committing.

## 4. Documentation and record

- [x] 4.1 `docs/cli.rst` (`solid test`, `:111-156`): the summary line's new
...
- [x] 4.2 `docs/testing.rst`: a short section — how to skip a test that does
  ...
  run.
- [x] 4.3 `docs/changelog.rst`: one line.
- [x] 4.4 `docs/adrs/TEST-FRAMEWORK/ADR-118-an-unexpected-success-fails-the-run.md`:
  accept ADR-118 per `design.md` §"Why this is an ADR", and add its row to
  `docs/adrs/README.md` in chronological order.
- [x] 4.5 `docs/architecture.md`, "Test framework (TEST-FRAMEWORK · spec
  ...
  new verdicts and ADR-118 — the overview is the reference, so it must
  reflect the accepted ADR.
- [x] 4.6 `workflow/warts.md`: mark the
...
- [x] 4.7 Sync the `test-framework` delta into `openspec/specs/` and archive
  the change.
