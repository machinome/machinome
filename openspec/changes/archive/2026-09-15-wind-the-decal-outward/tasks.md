...
- [x] 1.1 In `tests/test_markings.py`, add a `reversed_regions` helper that
...
- [x] 1.2 Add a `face_normals(vertices, faces)` helper built on
      `trimesh.Trimesh(..., process=False).face_normals` and
      `.triangles_center`, beside the existing `loaded` and
      `cylinder_departure` helpers.
- [x] 1.3 `WindingTest.test_a_wrapped_decal_faces_away_from_its_axis`: under
      ...
      faces into the drum.
- [x] 1.4 `WindingTest.test_a_flat_decal_faces_along_its_declared_normal`:
      ...
      RED: measured `-1.0`.
- [x] 1.5 `WindingTest.test_the_wrapped_decal_on_the_faceted_part_too`: the
...
- [x] 1.6 `WindingTest.test_reversed_artwork_builds_the_same_decal`: assert
      the three decals' `mesh_bytes` under 1.1 equal their `mesh_bytes`
      without it — the spec's "byte for byte" clause. RED under 1.1: the
      index order differs.
- [x] 1.7 `WindingTest.test_correct_artwork_is_untouched`: with NO patch,
...
- [x] 1.10 Run the file and record every failure message verbatim for the
      implementation commit's evidence.

## 2. The producer

- [x] 2.1 In `Svg.tessellate` (`solid_node/node/markings.py`), orient each
...
- [x] 2.2 In `Svg.__init__`, refuse a non-positive `scale` with `ValueError`
...
- [x] 2.3 Re-run `tests/test_markings.py`: group 1 green, nothing else moved.

## 3. Proof beyond the new tests

- [x] 3.1 Run the whole `tests/test_markings.py` file and confirm every
      ...
      its caller.
- [x] 3.2 Run `tests/test_sheet_leaf.py`, `tests/test_export.py` and
      `tests/test_node_lazy_exports.py` — the other users of build123d and
      of the marking's document path — and then the full suite.
- [x] 3.3 Rebuild the fixture decals from a cold build directory and confirm
      ...
      through `mesh_bytes`.
- [x] 3.4 Confirm no decal is re-derived by the change: build the fixture,
      note the artifact mtimes, apply nothing, build again, and confirm the
      recipe check leaves them alone (design D4).

## 4. Records

- [x] 4.1 `docs/markings.rst`, "What the build writes": one paragraph saying
...
- [x] 4.2 `docs/markings.rst`, the `Svg` reference: `scale` is positive.
- [x] 4.3 `HISTORY.rst`: fold one clause into the Unreleased marking entry —
...
- [x] 4.4 No ADR (design D5). No `workflow/` note (design, Open Questions).
