...
- [x] 1.1 In `tests/step_project/parts.py`, add two `StepNode` subclasses
...
- [x] 1.2 Add failing tests for each refusal: `part_index = 3` on a name two
...
- [x] 1.3 Add a failing test that `duplicate_names.step`'s inventory numbers
      ...
      after.
- [x] 1.4 In `tests/test_step_assembly.py`, add failing tests that
...
- [x] 1.5 In `tests/test_import_step.py`, add `build_dup_subassembly`
...
- [x] 1.6 Add a failing test that `generate_parts` over
      ...
      `Pin_2` (`evidence.md` §2).
- [x] 1.7 Add a failing test that COMPILES the `assembly.py` and `parts.py`
...
- [x] 1.8 Add a failing faithfulness test over `duplicate_names.step`
...
- [x] 2.1 Add `StepNode.part_index = None` with a docstring stating: 1-based,
...
- [x] 2.2 Rework `_select` to resolve `document.find(self.part)` once and
...
- [x] 2.3 Extend `_ambiguity_error` to name `part_index` as the way to
      choose, keeping the existing listing (`describe` per match) intact.
- [x] 2.4 Teach `_Document` which names are shared (one counter over
...
- [x] 3.1 Add `identity` and `part_index` to `ProductInfo` (`__slots__`,
      `__init__`, `__repr__`) and fill them in `StepAssembly.__init__`,
      counting same-named products over `document.order`.
- [x] 3.2 Add `product_identity` and `parent_identity` to `Occurrence`
...
- [x] 3.3 Confirm `Occurrence.identity` is untouched and still the COMPONENT
      label's entry; the reader's existing identity tests must pass
      unchanged.

## 4. The generator, keyed on identity

- [x] 4.1 `generate_parts`: key the returned mapping on
...
- [x] 4.2 `generate_assembly`: order and name assembly classes by identity
...
- [x] 4.3 Leave `_attribute_names` keyed on the product NAME (design D3) and
      add a comment saying why, so a later reader does not "fix" it.
- [x] 4.4 Confirm the part-import line (`sorted(set(class_names.values()))`)
      now names every generated class, including the ones a duplicate name
      previously stranded.

## 5. Generated source that parses

- [x] 5.1 Have `_placement_lines` report whether it emitted an operation,
...
- [x] 5.2 Confirm 1.7 goes green for every fixture, and that the generated
      source for `import_simple`, `import_nested` and `import_repeated` is
      byte-identical to today's.

## 6. Documentation, decision record, and validation

- [x] 6.1 Document `part_index` where `part` is documented (the `StepNode`
      ...
      inventory's `#N` shown.
- [x] 6.2 Document in the `import-step` CLI documentation that the command
      emits the selector for a shared name.
- [x] 6.3 Write ADR-115 (confirm the number is still free): a product is
      ...
      amended.
- [x] 6.4 Update `docs/architecture.md` where it describes STEP selection.
- [x] 6.5 Run `tests/test_step_node.py`, `tests/test_step_assembly.py` and
      `tests/test_import_step.py` green, then the full suite once.
- [x] 6.6 Mutation-check the two halves: reverting `_select`'s index lookup
      to `matches[0]` must fail 1.1; reverting `generate_parts` to
      name-keying must fail 1.6; removing the `pass` must fail 1.7.
- [x] 6.7 Record in `workflow/warts.md` that the YouCanBuildDog, orcahand
