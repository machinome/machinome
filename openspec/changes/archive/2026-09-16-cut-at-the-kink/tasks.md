...
- [x] 1.1 Re-run the five probes in this change's `spikes/` directory
      ...
      changing a line.
- [x] 1.2 RED (cost): `CurtaInterface` pays **2 861.3** `GraphValue`
      evaluations per tick and runs at **28.2 ticks/s**, against
      `Clearing`'s **98.6** and 1 528 and `Train`'s **8.0** and 2 288.
- [x] 1.3 RED (attribution): **84.5 %** of `CurtaInterface`'s evaluations
...
- [x] 1.4 RED (exactness): the crossing of `result0`'s rack in tick 17 of
...
- [x] 1.5 RED (blast radius): record the five corpus machines whose
...
- [x] 1.6 RED (the Curta): `running_probe --ticks 3` against this
...
- [x] 2.1 In `solid_node/simulation/program.py`, replace
...
- [x] 2.2 Keep `_affine_in_sources` as a thin "is it solvable" wrapper if
...
- [x] 2.3 PUBLICATION IS NOT CHANGED. `Program._published_edge`
...
- [x] 2.4 Unit tests over `_shape_of` directly, as a table: every
...
- [x] 3.1 One routine that, given a graph, the path's `start`/`delta`,
...
- [x] 3.2 A level that does not move over a sub-interval contributes no
      breakpoint, stated in the code as it is in `_Walk._searched`.
- [x] 3.3 Breakpoints are NOT crossings: assert in tests that they are
...
- [x] 4.1 `JumpPlan._crossings_of`: sub-divide a KINKED level's piece at
...
- [x] 4.2 `_Walk._crossing`: solve when the level and the skeleton are
...
- [x] 4.3 `Edge._affine_ends` reports the new shape; `_Block._affine_ends`
      keeps returning `False` for every give, with its comment saying WHY
      this change does not lift it (design.md §7).
- [x] 4.4 `Edge.cuts` returns the union of the plan's (or the walk's)
...
- [x] 4.5 `Run._locate` / `Run._piecewise`: unchanged in shape, exercised
      on the new cuts. Confirm no fourth tolerance appears anywhere.

## 5. The fixtures and the red tests

- [x] 5.1 `tests/running_project/machine.py`: a fixture whose driven
...
- [x] 5.2 RED then green, case C: the stop's fraction and the committed
...
- [x] 5.3 RED then green, case A (`tests/test_running_jumps.py`): a
      kinked jump level's crossing located within a few ulp of the
      closed-form answer.
- [x] 5.4 RED then green, case B (`tests/test_running_reads.py`): the
...
- [x] 5.5 Case E: `max(0, sin(x))` and a product-of-two-movers level pay
      exactly what they pay today.
- [x] 5.6 Case D: `Train` at exactly 8.0 evaluations/tick and `Clearing`
      at exactly 98.6, asserted, with `Clearing`'s committed values
      unchanged.
- [x] 5.7 Case D, the DOCUMENT (`tests/test_running_document.py`),
...
- [x] 6.1 Add ONE scenario over the 5.1 fixture to
...
- [x] 6.2 Regenerate with the tool (never by hand) and diff: expect
      ...
      not.
- [x] 6.3 Confirm the new scenario's `document` publishes the version the
...
- [x] 6.4 `tests/test_running_corpus.py` still green, `BlockOrderTest`
      included.

## 7. Performance

- [x] 7.1 Re-run `spikes/kink_baseline.py 5` and record the after table
...
- [x] 7.2 Re-run `tools/bench_selection.py 5`: all six numbers within
      the repeats' spread, `RangedBlock`'s 16.1 ms/tick included.
- [x] 7.3 Re-run `spikes/attribute_fixtures.py`: the two search rows are
      gone from `CurtaInterface` or reduced to the residue the solve
      leaves.
- [x] 7.4 Re-run the Curta's `running_probe --ticks 3` against the
...
- [x] 8.1 The five named regression files plus the corpus:
...
- [x] 8.2 The whole suite, sequential, one job:
...
- [x] 9.1 `docs/driving.rst`: which quantities the run SOLVES and which
...
- [x] 9.2 `docs/scenarios.rst`: sweep its refusal and limitation lists,
      as `read-the-driven-coordinate`'s own finding says a capability
      cycle must, and correct anything that says a kinked law is
      searched.
- [x] 9.3 `docs/architecture.md`: the Simulation section's account of the
...
- [x] 9.5 `workflow/warts.md`: the `read-the-driven-coordinate` entry "A
...
- [x] 9.6 AFTER implementation, under the reviewer's direction and in ONE
...
- [x] 10.2 Record, for the reviewer to file in `workflow/warts.md` and
