...
- [x] 1.1 Add to `tests/running_project/parts.py` nothing new; add to
...
- [x] 1.2 Add the refusal fixtures beside it: a continuous read
...
- [x] 1.3 Add `tests/clearing_project/` — a `CurtaInterface`-shaped
...
- [x] 2.1 RED: `tests/test_couplings.py` — a class stating
...
- [x] 2.2 GREEN: `motion/couplings.py` — `_refuse_shared_coordinate`
...
- [x] 2.3 RED then GREEN: a driven GROUP one of whose members the source
...
- [x] 2.4 RED then GREEN: `tests/test_couplings.py` — a self-read
...
- [x] 2.5 RED then GREEN: under a running root the relation is recorded
...
- [x] 2.6 RED then GREEN: a self-read relation with no rest default is
...
- [x] 3.1 RED: `tests/test_running_simulation.py` — constructing the
  fixture of 1.1 compiles. Fails today in `_ordered` with *"the relations
  … form a cycle the run cannot order"* (`evidence.md` §2).
- [x] 3.2 GREEN: `simulation/program.py` — `_relation_edge` registers the
...
- [x] 3.3 RED then GREEN: a law whose SKELETON still names the driven
  ...
  states.
- [x] 3.4 RED then GREEN: a self-read whose driven end is a plain port or
  a derived coordinate is refused at construction naming the port.
- [x] 3.5 GREEN by inspection AND test: `Program.described()` already
...
- [x] 4.1 RED: one tick of the fixture of 1.1 from a wheel at `108` with
...
- [x] 4.2 GREEN: `JumpPlan` gains the TWO-LAYER partition — `_partition`
...
- [x] 4.3 RED then GREEN: the crossing is recorded in `sim.crossings` and
  NOT in `sim.stops`.
- [x] 4.4 RED then GREEN: a second sweep of the same fixture leaves an
  already-cleared wheel BIT-IDENTICAL, and its command completes.
- [x] 4.5 RED then GREEN: ten constructions, one per digit `0 … 324`,
...
- [x] 4.6 RED then GREEN: the ACCURACY CONTRACT — one sweep taken in one
...
- [x] 4.7 RED then GREEN: both sweep directions; reversal with the wheel
  ...
  same float).
- [x] 4.8 A DOCUMENTATION test for the knife-edge gate (`wheel % 360 > 0`,
...
- [x] 4.9 RED then GREEN: a tick that would cut one self-read law's path
...
- [x] 4.10 RED then GREEN — the FAR-SIDE LANDING (F1). RED: with the
...
- [x] 4.11 RED then GREEN: a wheel standing EXACTLY on a band edge — the
...
- [x] 5.2 RED then GREEN: a declared range on the RING still blocks as
...
- [x] 5.3 RED then GREEN: a `Bound(..., reads=…)` one of whose reads is a
  self-read driven coordinate samples correctly over the sub-program.
- [x] 5.4 RED then GREEN: several wheels from one ring input disengage
  independently; an untouched wheel and an unswept register are unchanged.
- [x] 5.5 RED then GREEN: transactional refusal — a conflict in the
  segment after a self-read cut commits nothing, records no crossing, and
  retires the commands that moved `refused`.
- [x] 5.6 RED then GREEN: snapshot, restore, reset and bounded recording
...
- [x] 6.1 `tests/test_running_simulation.py`,
...
- [x] 6.2 `tests/test_running_corpus.py` replays the EXISTING committed
  fixture byte-identically before any regeneration.
- [x] 6.3 Measure `Train` before and after: the deterministic count of
...
- [x] 6.4 Run the whole suite once, sequentially, and record the count.

## 7. Publication

- [x] 7.1 RED then GREEN (`tests/test_running_document.py`): a root with
...
- [x] 7.2 RED then GREEN: a root with NO self-read law publishes a
...
- [x] 7.3 GREEN: `core/serializer.py` — the version ladder learns 6, and
  the CLI's existing "the installed viewer renders versions N…M" warning
  names it.
- [x] 7.4 RED then GREEN: `tools/generate_running_corpus.py` refuses a
...
- [x] 7.5 Regenerate `tests/running-corpus.json` with the new machines
...
- [x] 8.1 `docs/scenarios.rst` — a subsection under "A law that jumps"
...
- [x] 8.2 `HISTORY.rst` — one entry.
- [x] 8.3 `docs/architecture.md` — the "Simulation" section's running-mode
  paragraph and the "Known gaps and tensions" list.
- [x] 8.4 Extract **ADR-121** (NODE), "A law may read the coordinate it
...
## 10. Closure of the review's findings

The adversarial review of the implementation returned four findings. Each
was closed RED first, on this worktree, with the framework's own classes
and no monkeypatch; design.md's "Implementation notes (2026-09-15)" gives
the account and `evidence.md` §§18-21 the numbers.

- [x] 10.1 RED then GREEN: on a gate whose read changes the dial's RATE
  rather than stopping it, the same sweep at two cadences agrees inside
  the run's own window and cuts the path about as many times as there
  are surfaces between its ends, in BOTH directions. `_Walk._searched`
  no longer reports the surface a sub-interval STARTS on as a crossing
  of it, takes the surface nearest the left sample rather than the
  lowest, and takes a right sample exactly on a surface as the crossing
  there; `_bisect` says what it needs of `below`. `_decide` flips a node
  to the region IMMEDIATELY on the side its level departs to, which is
  the only reading of rule (c)'s "the OTHER branch's region" for a
  primitive with more than two.
- [x] 10.2 RED then GREEN: a dial engaged by a HAIR — resting a
  hundred-billionth of a degree short of its gap, on the searched shape
  and on the solved one — stops AT its gap rather than driving through
  it. `_searched` returns every crossing with `where > t`; rule (d) had
  been undone by a tolerance.
- [x] 10.3 GREEN: `_Walk._far_side` raises `LandingInvariantError`
  instead of committing the segment's own value when no bracket is
  found. Unreachable by construction, and said so in a comment where it
  raises and in `evidence.md` §20.
- [x] 10.4 design.md gains dated "Implementation notes (2026-09-15)"
  recording §10's unattainable `.repeat()` corpus scenario, §4's
  knife-edge prose that did not survive, and the three defects above;
  `evidence.md` §17 and `docs/scenarios.rst` re-checked against them.
