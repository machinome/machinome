...
- [x] 1.1 The baseline is already in `evidence.md` (section "The base
...
- [x] 1.2 In `tests/running_project/machine.py`, three fixtures — no new
...
- [x] 1.3 In `tests/test_running_document.py`, one test per spec
...
- [x] 1.4 In `tests/test_running_simulation.py`, that the RUN is
...
- [x] 2.1 In `compile_program` (`solid_node/simulation/program.py`),
      ...
      run.
- [x] 2.2 Extend `_Node`'s and `Program.nodes`' docstrings to say what
      the table now holds, and check that `published`'s docstring,
      `published_names`' and `_refuse_unqualified`'s still describe what
      they do.
- [x] 2.3 Nothing else in `program.py` changes. Confirm by reading, and
...
- [x] 3.1 Recapture `tests/base_documents/running_train.json` with
...
- [x] 3.2 Regenerate `tests/running-corpus.json` with
...
- [x] 3.3 Confirm the other six base documents are byte-identical and
...
- [x] 4.2 Re-run `evidence/probe_repeat_port.py`,
...
- [x] 4.3 Publish the originating project's machine: build
...
- [x] 5.1 `docs/architecture.md`: the running-program synthesis says what
      the compiled program's coordinates are.
- [x] 5.2 `docs/changelog.rst`.
- [x] 5.3 `workflow/warts.md`: mark the two findings fixed by this
      change, naming it, and leave the repeat-JOINT half filed with what
      `evidence/probe_repeat_joint.py` measured about it.
- [x] 5.4 Report for the pilot, outside this repository: the shop's
