...
- [x] 0.1 Work only in `solid-node/WTs/controls-on-parts` (branch
...
- [x] 0.3 Record the BASE per-tick cost by running
  ...
  answerable to.
- [x] 0.4 **Capture the base documents this cycle must not change.**
...
- [x] 0.5 Record the base `tests/running-corpus.json` digest
...
- [x] 0.6 Probe and paste, so the design's load-bearing facts are
...
- [x] 0.7 Grep the worktree and every project under
...
- [x] 1.1 `tests/running_project/parts.py`: `Dial`, a leaf with geometry
...
- [x] 1.2 `tests/running_project/machine.py`: `DialArbor`, an ASSEMBLY
...
- [x] 1.3 `tests/running_project/machine.py`: `Columns`, the fixture
...
- [x] 1.4 `tests/running_project/machine.py`: `OffCentre`, a running root
  whose posing joint is `Revolute(axis=(1, 0, 0), at=(0, 3, 0))`, for the
  `origin` case of design §8.
- [x] 1.5 `tests/running_project/machine.py`: one fixture per refusal, each
...
- [x] 1.6 `tests/running_project/machine.py`: `OmittedControl`, a running
...
- [x] 2.0 The names do not exist yet: `solid_node.simulation.Button`,
...
- [x] 2.1 **A control is declared and discovered.** New
  ...
  `sim.instructions`.
- [x] 2.2 **A control on a child qualifies through its path.** A root
  ...
  `column.Add one`.
- [x] 2.3 **The nearest posing joint wins.** On `Columns`, the control's
...
- [x] 2.4 **Every refusal of the `simulation` delta**, one test each,
...
- [x] 2.5 **A control changes nothing about the run.** `Columns` and a
...
- [x] 3.1 `solid_node/simulation/control.py` (new): `Button` and `Turn`,
...
- [x] 3.2 `solid_node/node/declarative.py`: `NodeMeta.__new__` validates
...
- [x] 3.3 `solid_node/simulation/enumeration.py`:
...
- [x] 3.4 `solid_node/simulation/__init__.py`: `Button` and `Turn` in
...
- [x] 4.1 **`Program.values_of` and `Program.deltas_of` are the run's
  ...
  needs.
- [x] 4.2 **`Program.response` reproduces the run.** Red first: for each
...
- [x] 4.3 **The corpus still replays.** `python -m pytest
...
- [x] 5.1 **The compiled control.** `compile_program` gains the controls
...
- [x] 5.2 **`origin` on an off-centre joint.** On `OffCentre`, `origin`
...
- [x] 5.3 **A site-declared joint carries.** A fixture whose joint is
...
- [x] 5.4 **The identity is untouched.** `Program.described()` is
...
- [x] 5.5 `solid_node/simulation/run.py`: `Run.__init__` hands
  `sim.controls` and `sim.instructions` to `compile_program`. Green for
  5.1–5.4.

## 6. Red first — the ratio

- [x] 6.1 **`per_unit` is the measured ratio.** Red first: on `Columns`,
...
- [x] 6.2 **An integer input measures over one native unit.** A fixture
...
- [x] 6.3 **Zero is refused.** A `Turn` whose input reaches the
...
- [x] 6.4 **A two-sided disagreement is refused.** A `Turn` over a law
...
- [x] 6.5 `Program.published_controls(initial)` implementing 6.1–6.4.
  Green.

## 7. Red first — the document

- [x] 7.1 **The table is published.** In
...
- [x] 7.2 **Absent when empty, and byte-identical.** `Train`,
...
- [x] 7.3 **The version does not move.** `Columns` declares
...
- [x] 7.4 **Republishing is byte-identical**, key order and field order
...
- [x] 7.5 **An omitted part drops its control.** On `OmittedControl`
...
- [x] 7.6 **A baked capture publishes none.**
  ...
  otherwise unchanged.
- [x] 7.7 `solid_node/core/serializer.py`: `document_body` gains a
...
- [x] 8.1 `python -m pytest tests/test_running_document.py
  ...
  counts.
- [x] 8.2 `python -m pytest tests/test_cli_lazy_imports.py
...
- [x] 8.3 **Regenerate the corpus and prove `identity` is unchanged.**
...
- [x] 8.4 Full suite (`python -m pytest -x -q`), counts pasted beside
  0.2's, every difference accounted for by a test this cycle added or
  rewrote.

## 9. Documentation and the record

- [x] 9.1 `docs/driving.rst`: a "Controls: pressing and turning the part
...
- [x] 9.2 `docs/scenarios.rst`: one cross-reference from the running
  ...
  sections.
- [x] 9.3 `python -m pytest tests/test_docs_exports.py -q` green, and
  the docs build if the suite does not already cover it.
- [x] 9.4 `evidence.md` complete: the base and final suite counts, the
  base and final corpus digests, every red text, the probes of 0.6, the
  grep of 0.7, and the cost probe before and after task 4.
- [x] 9.5 Update `workflow/docs/controls-on-parts.md` to point at this
...
- [x] 9.6 Report to the reviewer, do NOT commit, and do NOT extract the
