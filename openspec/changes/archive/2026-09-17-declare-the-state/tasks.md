...
- [x] 1.1 Add `tests/clocked_project/` with the REGISTER fixture: a `Counter`
...
- [x] 1.2 Add the CLEARING fixture beside it, Curta-shaped and tiny: a root
  ...
  fixture form.
- [x] 1.3 Add the refusal fixtures: a non-`State` target; one relation
...
- [x] 2.1 RED: a test asserting `State(default=0, range=(0, 9), dtype=int)`
  is declarable, readable as `self.units`, and carries the declaration's five
  fields off the class. Fails: no `State`.
- [x] 2.2 GREEN: `solid_node/simulation/state.py` beside `driver.py` — the
...
- [x] 2.6 GREEN: each refusal at the site that owns its facts, per
  design §2.

## 3. The verb (`commits`)

- [x] 3.1 RED: the declaration tests — a bare `commits` recorded on the
  ...
  Fails: no `commits`.
- [x] 3.2 GREEN: `commits` on `CoordinateRef` and `Coordinates` in
...
- [x] 3.3 RED: the class-definition refusals of 1.3 that belong to the verb
...
- [x] 3.4 GREEN: those refusals, each naming the relation as written and the
  class that stated it.

## 4. Compiling `at` and `law`

- [x] 4.1 RED: `at` accepted as one `floor`, one `ceil`, one `sign` and one
  comparison; refused as a compound, as `%`, as bare arithmetic, and as raw
  text or a call outside the vocabulary.
- [x] 4.2 GREEN: the `at` inspection — `_graph_of`'s walk reused for text and
...
- [x] 4.3 RED: a commit law made ENTIRELY of jumps (`floor(crank / 360) % 10`)
...
- [x] 4.4 GREEN: the law inspection — `_graph_of`'s text and vocabulary
  walk, with NO jump plan, NO skeleton and NO `_only_jumps` refusal, because
  a commit is evaluated at a point.
- [x] 4.5 RED: a curved `at` is refused AT SIMULATION CONSTRUCTION naming the
  relation, the driver whose motion curves the level, and the primitive.
- [x] 4.6 GREEN: the classification — the level graph bound with every
...
- [x] 5.1 RED: a solver unit test over the register fixture's own level —
...
- [x] 5.2 GREEN: `solid_node/simulation/clocked.py` — a CONSUMER of
...
- [x] 5.3 RED: the landing — a crossing of `floor(crank / 360)` at
...
- [x] 5.4 GREEN: the landing through the existing `_far_side`/`_ordinal`
  float walk, whose membership test is `_branch_of` at the candidate value
  and never a comparison against the surface (design §5).
- [x] 5.5 RED: a KINKED level (`floor(max(crank, 0) / 360)`) is SOLVED at its
  breakpoint against a hand-computed fraction, and the breakpoint is not
  reported as an event.
- [x] 5.6 GREEN: the kink sub-division, `_KinkCuts` reused.
- [x] 5.7 RED: simultaneity by IDENTITY — two relations whose far-side
...
- [x] 5.8 GREEN: the tie merge — equality of the landing float, no tolerance
  — and the synchronous read.
- [x] 5.9 RED: an event surface that reads its own state MOVES after the
  commit (the clearing fixture: one event on the first sweep, none on the
  second).
- [x] 5.10 GREEN: re-solving the remaining path from the landing with the new
  bank.
- [x] 5.11 RED: more events than the crossing maximum is refused committing
  ...
  is ADMITTED.
- [x] 5.12 GREEN: the bound and its refusal, `_MAX_CROSSINGS` (1000 per
  graph) reused, counted as events are located.

## 6. The clocked `Sim`

- [x] 6.1 RED: `Sim(node)` over the register fixture constructs, poses once,
  ...
  today.
- [x] 6.2 GREEN: the constructor split in `sim.py`, entered only when the
  state table is non-empty.
- [x] 6.3 RED: `sim.move('crank', by=360)` → one event, `units == 1`;
...
- [x] 6.4 GREEN: the request path, the `Request`/`Commit` value objects, and
  the ONE binding of the tree at the end of the request.
- [x] 6.5 RED: atomicity — a law raising on the third event leaves the bank,
  the tree and the record exactly as they were.
- [x] 6.6 GREEN: staged commit, applied only when the whole request succeeds.
- [x] 6.7 RED: `state=` accepting a state id; `snapshot`/`restore`/`initial`/
...
- [x] 6.8 GREEN: those, on the bank.
- [x] 6.9 RED: each of `run`, `at`, `every`, `time`, `tick`, `rate`,
  `trigger`, `commands`, `program`, `crossings`, `stops` refused by name on a
  clocked `Sim`.
- [x] 6.10 GREEN: the refusals, each naming the clocked base.
- [x] 6.11 RED: a request whose final pose violates a joint `range` raises
  ...
  half a path.
- [x] 6.12 GREEN: pose the working bank BEFORE assigning it, re-pose the
...
- [x] 6.13 RED: UNITS — a state declaring `scale=10` commits the value its
  ...
  (design §9).
- [x] 6.14 GREEN: the commit writes the law's NATIVE return, with a single
  `round()` for a `dtype=int` target and no scale conversion anywhere.
- [x] 6.15 RED: TIME — a clocked fixture whose `simulate()` reads `self.time`
...
- [x] 6.16 GREEN: nothing to implement — the clocked pose takes the build
...
- [x] 7.1 RED: serializing the register fixture is refused naming `units`,
  ...
  - A stateless fixture's document stays byte-identical to its recorded
    bytes.
- [x] 7.2 GREEN: the refusal in `core/serializer.py`, placed in
...
- [x] 8.1 RED: a counter on the clocked collector asserted ZERO for a driven,
  stateless model constructed, posed, stepped with `Sim(node, dt)` and
  published.
- [x] 8.2 GREEN: the guard — clocked paths entered only on a non-empty state
  table, states collected in the existing walk, exports left lazy.
- [x] 8.3 The whole existing suite green, including the running fixtures and
  `tests/running-corpus.json`, with no fixture edited and no expected value
  changed.

## 9. Measurement

- [x] 9.1 Measure on the two fixtures ONLY and record in `evidence.md`:
...
- [x] 9b.1 RED (C1): two children of ONE class each declaring `digit`, one
...
- [x] 9b.2 RED (C2): one state written by TWO relations at two DIFFERENT
  ...
  `sim.state` unchanged.
- [x] 9b.3 GREEN (C1/C2): drop `declarative._refuse_two_writers` and the
...
- [x] 9b.4 The Curta-shaped `Register` fixture in `tests/clocked_project/`:
  ...
  continue from zero.
- [x] 9b.5 RED (C3): a request whose FINAL POSE raises `JointRangeError`
  leaves the bank advanced today. It must leave the bank, the tree and the
  record exactly as they were; the same for `restore()`.
- [x] 9b.6 GREEN (C3): pose the working bank first in `Clocked.move` and
  `Clocked.restore`, assign only on success, re-pose the previous bank and
  re-raise on failure.
- [x] 9b.7 RED (C4): a committing relation whose sources are ALL states
...
- [x] 9b.8 GREEN (C4): the refusal in `_compiled`, beside the curved-level
  one.
- [x] 9b.9 The five clocked modules green, then the FULL suite once.

## 10. Records

- [x] 10.1 `docs/scenarios.rst` and `HISTORY.rst`.
- [x] 10.2 Extract ONE ADR, candidate **ADR-125** (NODE), AFTER implementation
  ...
  changed.
- [x] 10.3 Amend `workflow/docs/clocked-machine.md` — the requirement note —
...
- [x] 10.4 Record in `workflow/warts.md` the follow-ups this cycle names: a
...
- [x] 10.5 Sync the baseline specs, archive the change, and commit the
