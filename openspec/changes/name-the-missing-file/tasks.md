...
- [ ] 1.1 Extend `tests/stl_project/parts.py` with two classes that declare a
  ...
  having run.
- [ ] 1.2 Add `tests/test_missing_source_file.py`. Against the current code
...
- [ ] 1.3 Run the new file and record which assertions are red and why:
  `PYTHONPATH="$PWD" .venv/bin/python -m pytest tests/test_missing_source_file.py -x -q`.

## 2. Make it green

- [ ] 2.1 Add `require_source_file(klass, attribute, declared, path)` to
...
- [ ] 2.2 Call it from each adapter immediately after the path is resolved
  and before `super().__init__`: `stl.py` (after line 170), `step.py` (after
  484), `jscad.py` (after 34), `openscad.py` (after 41, i.e. before
  `coherent_read` at 45).
- [ ] 2.3 Re-run `tests/test_missing_source_file.py` green.

## 3. Prove nothing else moved

- [ ] 3.1 `tests/test_stl_node.py`, `tests/test_step_node.py`,
...
- [ ] 3.1a `tests/test_builder_reload_resilience.py` — green, and read the
...
- [ ] 3.1b Add to `tests/test_builder_reload_resilience.py` a variant of
...
- [ ] 3.2 `tests/test_cli.py` and `tests/test_manager_new.py` — green: the scaffold writes a `Solid2Node` and
  must be untouched.
- [ ] 3.3 The whole suite once before the implementation commit.
- [ ] 3.4 Re-run `evidence/run-probes.sh` and record the new messages beside
  the old ones in `evidence.md` under a "After" heading, so the before/after
  is one document.

## 4. Documentation and record

- [ ] 4.1 `docs/leaf-nodes.rst`: one sentence in each of the four adapters'
...
- [ ] 4.2 `docs/changelog.rst`: one line.
- [ ] 4.3 `workflow/warts.md`: mark the
...
- [ ] 4.4 Sync the three spec deltas into `openspec/specs/` and archive the
  change.
