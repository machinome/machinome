...
- [ ] 1.2 In `tests/running_project/machine.py`, one fixture pair beside
...
- [ ] 1.3 In `tests/test_running_document.py`, one test per new spec
...
- [ ] 1.4 In `tests/test_running_simulation.py`, that a LIVE run is
...
- [ ] 2.1 In `symbolic_document` (`solid_node/core/serializer.py`),
...
- [ ] 2.2 Comment it with WHY, in the module's own voice: the walk's own
...
- [ ] 2.3 Extend `symbolic_document`'s docstring — the paragraph that
      ...
      none.
- [ ] 2.4 Nothing in `solid_node/motion/couplings.py`,
...
- [ ] 3.1 Confirm every base document under `tests/base_documents/` is
...
- [ ] 3.2 Confirm `tests/running-corpus.json` is unchanged by
...
- [ ] 4.2 Re-run `evidence/probe_publication.py`, `evidence/probe_sim.py`
...
- [ ] 4.3 Publish the originating project's machine: re-run
...
- [ ] 5.1 `docs/architecture.md`: the publication synthesis says what the
      producer puts back.
- [ ] 5.2 `docs/changelog.rst`.
- [ ] 5.3 `workflow/warts.md`: mark the finding FIXED with the cycle's
...
- [ ] 5.4 No ADR: this restores a stated contract and opens no new
