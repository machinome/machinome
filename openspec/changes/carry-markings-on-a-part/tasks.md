...
- [ ] 1.1 Create `tests/markings_project/` as a package (`__init__.py`,
...
- [ ] 1.2 Add the fixture parts across three modules, in BOTH leaf kinds the
...
- [ ] 1.3 Add `assembly.py` placing both parts under an `AssemblyNode` with a
  `Revolute` joint on the dial, so the ride-the-placement scenario and the
  document tests have one tree to read.
- [ ] 1.4 Add `tests/test_markings.py` with the fixture plumbing the other
...
- [ ] 2.1 **Red**: a rigid leaf declaring `digits = Marking(Svg(...),
...
- [ ] 2.2 Write `solid_node/node/markings.py` with `Marking`, `Svg`, `Wrapped`
...
- [ ] 2.3 **Red**: inheritance and dropping — a subclass inherits the base's
  ...
  stubbing the walk.
- [ ] 2.4 **Red**: `AssemblyNode` and `FlexibleNode` subclasses declaring a
...
- [ ] 2.5 **Red**: a marking whose attribute name is also a declared parameter,
  ...
  coordinates).
- [ ] 2.6 **Red**: a marking named for an attribute every node already carries
  ...
  the same way.
- [ ] 2.7 **Red**: a `Marking` with a bad `color` raises `ValueError` at class
...
- [ ] 2.8 Extend `NodeMeta.__new__` (`declarative.py:798-826`) with
  ...
  green.
- [ ] 2.9 **Red**: `from solid_node.node.markings import Marking, Wrapped,
...
- [ ] 3.1 **Red**: an `Svg` whose path does not exist is refused at class
...
- [ ] 3.2 **Red**: the fixture SVG reduces to its closed regions with holes
...
- [ ] 3.3 **Red**: `Svg('...', scale=25.4)` multiplies every artwork
  coordinate.
- [ ] 3.4 **Red**: `Flat` — points land on the declared plane, artwork X runs
  ...
  exists.
- [ ] 3.5 **Red**: `Wrapped` — every point at the declared radius about the
...
- [ ] 3.6 **Red**: the angular zero. With `start=0` and the artwork origin at
...
- [ ] 3.7 **Red**: `pitch=36` stamps the artwork ten times at 36° intervals;
...
- [ ] 4.1 **Red**: building the fixture writes one marking artifact per
...
- [ ] 4.2 **Red**: the wrapped decal follows its cylinder — no vertex departs
...
- [ ] 4.3 **Red**: the decal sits on the nominal surface with NO offset —
  every vertex at exactly the declared radius / on exactly the declared plane,
  to floating-point tolerance.
- [ ] 4.4 Add the artifact path and the build to `AbstractBaseNode`
  ...
  process.
- [ ] 4.5 **Red**: the marking artifact records a producer recipe — a build
  ...
  recipe.
- [ ] 4.6 **Red**: the two halves that distinguish this design from the sheet
...
- [ ] 4.7 **Red**: editing the ARTWORK file regenerates only the marking —
...
- [ ] 4.8 **Red**: editing the DECLARATION (the placement numbers) regenerates
...
- [ ] 5.1 **Red**: two classes of the same name and the same parameters, one
...
- [ ] 5.2 **Red**: `assertNoIntersectingSolids`, `assertNoDisconnectedSolids`
...
- [ ] 5.3 **Red**: a rigid node declaring `digits` then `arrows` publishes a
...
- [ ] 5.4 **Red**: a marking entry's `mtime` moves when its artwork is edited
  and the node's own `mtime` and `model` do not.
- [ ] 5.5 Add the keyword-only `marking_path=None` callback to
...
- [ ] 5.6 **Red**: byte-identity, WITHOUT recapturing anything. The seven
...
- [ ] 5.7 **Red**: export copies every named marking under `models/`,
...
- [ ] 5.8 **Red**: the sweep keeps every marking the snapshot names, and
...
- [ ] 5.9 **Red**: the OpenSCAD path is unaffected — a model whose parts
...
- [ ] 6.1 Write `docs/markings.rst` — what a marking is and is not, the
...
- [ ] 6.2 Link it from `docs/index.rst`, and add the cross-reference from
...
- [ ] 6.3 Add the `HISTORY.rst` Unreleased entry: what a marking is, that it
  adds no solid, no part and no piece, the additive document growth, and that
  the viewer draws it in its own release.
- [ ] 6.4 Run the full suite plus a real caller —
  `projects/Calculators/Pascaline-module` — and record both results.
- [ ] 6.5 AFTER implementation, extract the ADR: a marking is a declaration on
