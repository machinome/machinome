...
- [ ] 1.1 Add the cross-package fixture `tests/cross_package_project/`:
...
- [ ] 1.2 Add `tests/test_scad_import_paths.py` with a helper that reads a
...
- [ ] 1.3 Add to `tests/test_snapshot.py`: with a fake runner returning
...
- [ ] 1.4 Record the red run (the failing assertions and their messages) in
  `evidence.md` under "Red".

## 2. Make it pass

- [ ] 2.1 `solid_node/node/base.py`: add `_ArtifactImport(import_stl)`, the
...
- [ ] 2.2 Route the remaining emitters through `artifact_import`:
  `exact_leaf.py:99`, `flexible.py:324` (the per-binding snapshot),
  `adapters/jscad.py:94`, `adapters/stl.py:201`.
- [ ] 2.3 `adapters/openscad.py`: its `scad_code` override renders the same
  re-anchored model, so an `OpenScadNode` is not a hole in the rule.
- [ ] 2.4 Confirm `self.root` is no longer read for any path; leave the
  attribute and its propagation alone if anything else uses it, and say in
  the commit which.
- [ ] 2.5 `solid_node/viewers/openscad.py`: log every warning, error or
  ...
  the code.
- [ ] 2.6 `solid_node/manager/snapshot.py`: catch that error, write the
  message to stderr, exit 1, and leave no image behind.
- [ ] 2.7 Run the section 1 tests green; then
...
- [ ] 2.8 Re-run `evidence/probe_cross_package.py` against the fixed tree and
  append its output to `evidence.md` under "Green", showing every import
  resolving in every generated `.scad`.
- [ ] 2.9 Measure `_model_for_own_scad` on the largest assembly fixture the
...
- [ ] 3.1 `docs/adrs/BUILD/ADR-116-an-artifact-import-is-anchored-on-the-build-directory.md`
...
- [ ] 3.2 Update `docs/architecture.md` where it describes the build
  artifact layout, so the anchoring rule is part of the described system.
- [ ] 3.3 Documentation: wherever the docs explain a project's package
  layout or the generated SCAD, state that an assembly may live in any
  package relative to the parts it places.
- [ ] 3.4 `workflow/warts.md`: mark the Thor bullet "A leaf's artifact is
...
- [ ] 3.5 Full suite: `python -m pytest tests -q`, recorded in `evidence.md`
  with the count and any pre-existing failures identified as pre-existing.

## 4. Close the cycle (reviewer)

- [ ] 4.1 `openspec validate import-the-artifact-by-path --strict`, sync the
