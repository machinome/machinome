...
- [ ] 1.1 In `tests/test_manager_test.py`, add a failing test that drives the
...
- [ ] 1.2 Add a failing test for the LOST face (`evidence.md` §7): after a
...
- [ ] 1.3 Add a running fixture to `tests/running_project/machine.py` whose
      ...
      of four.
- [ ] 1.4 In `tests/test_joints.py`, add a failing test that binds a joint,
      ...
      its placement).
- [ ] 1.5 Add a test that every joint kind's placement carries its
...
- [ ] 1.6 RED FIRST for revision 1's Finding A. Add an UNTIMED fixture
...
- [ ] 1.7 Add the green-before-and-after controls: an untimed `TrainBody` run
...
- [ ] 2.1 Change `Joint.clear` (`solid_node/motion/joints.py:738`) to drop
...
- [ ] 2.2 Add the module-level seam that re-places every joint a node
...
- [ ] 2.3 Move `CoordinateDelivery.restore`
...
- [ ] 2.4 Run `tests/test_joints.py`, `tests/test_motion_package.py`,
...
- [ ] 3.1 Have `restore_children_checkpoints`
...
- [ ] 3.2 Run `tests/test_manager_test.py`; 1.1-1.3 turn green, 1.7 stays
...
- [ ] 4.0 Change `clear_solved` (`solid_node/motion/couplings.py:1786`) so
...
- [ ] 4.1 Run `tests/test_couplings.py`, `tests/test_joints.py`,
...
- [ ] 4.2 Add the untimed HAND-binding test of `evidence.md` §6/§A5: the
...
- [ ] 5.1 Re-run both probes and the fixture's `solid test`
      ...
      coordinates.
- [ ] 5.2 Re-run the wart's own reproduction on the lock
      (`evidence/probe_lock.py`) and record it.
- [ ] 5.3 Write the ADR, numbered next in `docs/adrs/`, and add it to the
...
- [ ] 5.5 Update `workflow/warts.md`: mark the pin tumbler lock's first bullet
