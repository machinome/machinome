...
- [ ] 1.1 Add `tests/carriage_project/` (`__init__.py` and `machine.py`;
...
- [ ] 1.2 Add the CURTA-SHAPED fixture to the same package: four dials on
...
- [ ] 1.3 Add the REFUSAL fixtures beside them: the reduced fixture with
...
- [ ] 2.1 RED: `tests/test_running_simulation.py` — the reduced fixture
...
- [ ] 2.2 RED: a test that the unconditional cycle of 1.3 WITH ITS
...
- [ ] 2.3 GREEN: `program.py` — the union dependency graph over the
...
- [ ] 2.4 GREEN: the SELECTOR classification on each law edge's plan —
  ...
  unconditional.
- [ ] 2.5 RED then GREEN: the construction refusals of design.md §2,
...
- [ ] 2.6 RED then GREEN: `Program.described()` gains one `block` line
...
- [ ] 3.1 RED: the reduced fixture's rest bank is expected to be each
...
- [ ] 3.2 GREEN: `program.py` gains `_block_members(root)`, a pre-pass
...
- [ ] 3.3 RED then GREEN: a block relation with no rest default is
  refused naming the qualified id, with the run's existing message for
  an unbound joint coordinate.
- [ ] 3.4 RED then GREEN: under no time base and under `Time(loop=4)`
...
- [ ] 3.5 GREEN: assert the pre-pass's membership and the compile's
  ...
  today.
- [ ] 3.6 RED then GREEN: the marks do not outlive a simulation. The
  ...
  that true.
- [ ] 3.7 RED then GREEN: the two moved refusals of design.md §5 — the
...
- [ ] 4.1 RED: the reduced fixture cranked with `shift` at `0`, and
...
- [ ] 4.2 GREEN: the compound `block` `Edge` — union `needs`, all
  ...
  `0.5`.
- [ ] 4.3 RED then GREEN: a unit test that a member's plan evaluated with
...
- [ ] 4.4 RED then GREEN: `tests/test_running_jumps.py` — a selector
  ...
  `sim.stops`.
- [ ] 4.5 RED then GREEN: a selection change alone moves nothing —
  ...
  for bit.
- [ ] 4.6 RED then GREEN: the ACCURACY CONTRACT — the same crank travel
...
- [ ] 4.7 RED then GREEN: a LANDING followed by MOTION in a later piece
...
- [ ] 4.8 RED then GREEN: a member evaluated on a piece with an in-block
...
- [ ] 5.5 RED then GREEN: the Curta-shaped fixture's interlock — a
...
- [ ] 5.6 RED then GREEN: RETENTION — shift away and back preserves
...
- [ ] 6.1 RED then GREEN: the fixture of 1.3 whose two selections are
...
- [ ] 7.1 RED then GREEN: `tests/test_running_reads.py` — a block member
...
- [ ] 7.2 GREEN: assert that the `_Retained` split is decided ONCE at
...
- [ ] 8.1 RED then GREEN: `tests/test_running_document.py` — a program
...
- [ ] 8.2 GREEN: `serializer.py` — `BLOCK_DOCUMENT_VERSION = 7`,
...
- [ ] 8.3 RED then GREEN: a block's members publish as ORDINARY law
...
- [ ] 8.4 GREEN: `tools/generate_running_corpus.py` — three new
...
- [ ] 8.5 RED then GREEN: `tests/test_running_corpus.py` — the generator
...
- [ ] 8.6 Regenerate `tests/running-corpus.json` through
...
- [ ] 9.1 Run `tests/test_running_simulation.py`,
...
- [ ] 9.2 Measure the cost, per design.md §10: the `Train` bench as the
  ...
  change's evidence.
- [ ] 9.3 `docs/driving.rst` and `docs/scenarios.rst`: a section on a
...
- [ ] 9.4 `HISTORY.rst`: the Unreleased entry, in the voice of the
  ...
  version 7 sentence.
- [ ] 9.5 `workflow/docs/curta-shifted-carry-association.md`: the status
  line becomes "Taken up as `select-the-source`", with the ADR number
  once it exists.
- [ ] 9.6 AFTER implementation, under the reviewer's direction, and in
...
- [ ] 10.2 Record that MIGRATING THE CURTA is another agent's project
