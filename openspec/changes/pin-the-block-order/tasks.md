...
- [ ] 1.1 Copy `spikes/order.py`, `spikes/patched_corpus.py`,
...
- [ ] 1.2 RED (the replay): with `_Block._order` patched to return the
      ...
      members reversed.
- [ ] 1.3 RED (the whole corpus): under the same patch every one of the
...
- [ ] 1.4 RED (the guard): `uncovered_features` over the committed
...
- [ ] 2.1 In `tools/generate_running_corpus.py`, change the
...
- [ ] 2.2 Update the entry's comment to say what the script is FOR — the
...
- [ ] 2.3 Regenerate `tests/running-corpus.json` with the tool (never by
      ...
      Record the diff.
- [ ] 2.4 Confirm the regenerated `ShiftedCarry` entry's `document` is
...
- [ ] 3.1 Add `'an in-block gate crossing inside a tick'` to `REQUIRED`,
      after `'a tick carrying both a selection crossing and a stop'`.
- [ ] 3.2 Derive it in `uncovered_features` as design.md §2 states:
...
- [ ] 3.3 Keep the helper small and beside `_selection` and `_member_of`,
      reusing `free_names`; add no new import and no new document key.
- [ ] 3.4 GREEN: `uncovered_features` over the regenerated corpus returns
      `[]`, and it returns the new feature for a corpus with
      `ShiftedCarry` removed.

## 4. The tests

- [ ] 4.1 In `tests/test_running_corpus.py::CoverageGuardTest`, add
...
- [ ] 4.2 Add a second narrowed case in the shape of
...
- [ ] 4.3 Add `BlockOrderTest` (design.md §3): replay the `ShiftedCarry`
...
- [ ] 4.4 Prove 4.3 red first: run it against the committed corpus and
...
- [ ] 4.5 Run `tests/test_running_corpus.py` and
      `tests/test_running_document.py` green, and report the counts.
- [ ] 4.6 Run the named running and export suites
...
- [ ] 5.1 Sync the `export` delta into
...
- [ ] 5.2 One `HISTORY.rst` line under Unreleased, in the voice of the
      existing entries: the corpus now catches a consumer that runs a
      block's members in the order the document lists them.
- [ ] 5.3 In `workflow/warts.md`, under the `select-the-source` section,
...
- [ ] 5.4 Note in the cycle's report, for the viewer's own cycle: the
...
- [ ] 5.5 `openspec validate pin-the-block-order --strict`; the reviewer
      archives after review.
- [ ] 5.6 A dated note in `docs/adrs/NODE/ADR-122-...md`'s Consequences,
