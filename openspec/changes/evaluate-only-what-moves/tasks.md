...
- [ ] 1.1 Run `spikes/path_baseline.py 3` from the worktree root with
...
- [ ] 1.2 RED (the wart): from the originating project, read-only,
...
- [ ] 1.3 RED (attribution): `spikes/split_curta.py 3` and
      ...
      **77.6 % of the tick is the two `_Walk._searched` sites** — while
      ...
      1 % of the seconds.
- [ ] 1.4 RED (the decisive number): `spikes/cone_curta.py`. Confirm the
...
- [ ] 1.6 RED (end to end): `spikes/endtoend_curta.py plain 3` and
...
- [ ] 1.7 RED (the suite): time `simulation/test_running.py` and
...
- [ ] 1.8 RED (the rejected alternative, measured): `spikes/perpiece_curta.py`.
...
- [ ] 2.1 Add `_PathValue` to `solid_node/simulation/program.py`, beside
      ...
      without walking.
- [ ] 2.1a The FIRST point of a piece is the build (design.md §3.1): the
...
- [ ] 2.2 The docstring states the contract in the spec's words: what
...
- [ ] 2.3 The moving set is taken from the run, never inferred: a source
...
- [ ] 2.4 NO cache outlives the tick (design.md §3.3, measured at 5 %).
...
- [ ] 3.1 `_Walk`: build the skeleton's path value and one per dependent
...
- [ ] 3.2 `JumpPlan`: `_level`, `_level_at`, `_branches`, `_solved` and
      `_searched` take path values built once per `increment` and re-bound
      per piece on `inner`.
- [ ] 3.3 `_skeleton_cuts` / `_level_cuts` (`_KinkCuts.between`) are LEFT
...
- [ ] 3.4 Leave alone, and say so in a comment where it is not obvious:
...
- [ ] 4.1 `tests/base.py`: `expression_evaluations` must count a path
...
- [ ] 4.2 `tests/base.py` gains `graph_node_visits(sim, ticks)` beside
...
- [ ] 5.1 `tests/clearing_project/machine.py` gains the fixture of
...
- [ ] 5.2 RED (case A): `graph_node_visits` per tick of that fixture,
...
- [ ] 5.3 RED (case B): a golden — the fixture's committed bank, its
...
- [ ] 5.4 RED (case D): a test-only checking path value in `tests/` —
      ...
      otherwise.
- [ ] 5.5 RED (case C): `Clearing` at exactly 98.6 evaluations/tick and
...
- [ ] 5.6 A piece's branches do not leak: a fixture whose tick is cut
...
- [ ] 6.1 `tests/running-corpus.json` byte-identical for all 19
      scenarios (`git diff --stat` empty), and `tests/test_running_corpus.py`
      green including `BlockOrderTest`.
- [ ] 6.2 `Program.published()` byte-identical for every running fixture;
      no document field, no `affine` flag, no version.
- [ ] 6.3 `tools/bench_selection.py 5`: the six numbers within their
      repeats' spread, `RangedBlock` at 16.0 ms/tick.
- [ ] 6.4 `spikes/path_baseline.py 3` after: `Train` 8.0 evaluations and
...
- [ ] 6.5 The five named regression files, then the whole suite,
...
- [ ] 7.1 `running_probe --ticks 3` from the originating project,
      read-only: **≤ 1.20 s per 0.1-s tick** (expected ≈ 0.73), against
      the 3.27 s of task 1.2.
- [ ] 7.2 `spikes/endtoend_curta.py plain 3` after the change must give
...
- [ ] 7.3 `simulation/test_running.py` + `simulation/test_running_clearing.py`
      ...
      that baseline.
- [ ] 7.4 Construction is NOT claimed: record it and say so. If it has
      moved outside its spread either way, say which way and why.

## 8. Documentation

- [ ] 8.1 `docs/architecture.md`, Simulation section: one paragraph on
      what a tick evaluates and what it computes once, beside the
      jump-plan and self-read accounts. Add `_PathValue` to the Map.
- [ ] 8.2 `docs/architecture.md`, "Known gaps and tensions": the
...
- [ ] 8.3 `docs/driving.rst`: extend `cut-at-the-kink`'s "What a law
...
- [ ] 8.4 `HISTORY.rst`: an Unreleased entry in the voice of the two
      above it. No breaking note; no document version moves.
- [ ] 8.5 `workflow/warts.md`: Astra's "Originating Curta follow-up:
...
- [ ] 8.6 A finding for solid-node-viewer: its TypeScript run has the
...
- [ ] 9.1 Draft `docs/adrs/NODE/ADR-124-only-what-moves-along-a-tick-s-path-is-evaluated.md`
...
- [ ] 9.2 Promotion is the reviewer's direction: leave it a DRAFT until
      the cycle's review says so.

## 10. Findings

- [ ] 10.1 Record in `workflow/warts.md`, under this cycle, anything met
