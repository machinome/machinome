...
- [ ] 1.1 Record the green baseline: run the whole suite and keep the summary
      and the per-test timings of the running fixtures.
- [ ] 1.2 Record `git status --short`, the hash of `tests/running-corpus.json`
      and the hash of every file under `tests/running_project/`, so
      "untouched" is a checked fact at the end and not a claim.
- [ ] 1.3 Publish and keep the FULL document body of at least four stateless
      ...
      section 11.
- [ ] 1.4 Verify against the SOURCE and record: every producer that reaches
...
- [ ] 2.1 RED: a test asserting that a compiled committing relation exposes
...
- [ ] 2.2 Implement: `Committing` retains the checked law graphs, one per
...
- [ ] 2.3 RED then green: `CompiledClocked.published(initial)` — the whole
...
- [ ] 2.4 Implement `commits` entries (design §6): `sources`, `targets`,
...
- [ ] 2.4a RED: a law over a NEGATIVE operand banks one value and a verbatim
...
- [ ] 2.4b Implement the DESUGARING of design §16: every `%` node of a
...
- [ ] 2.4c Pin the INTEGER ROUNDING a commit applies, as a test naming what
...
- [ ] 2.5 Implement `bounds` entries (design §7): `coordinate`, `side`,
...
- [ ] 2.6 Implement the placeholder mint for the bounds' plans through the
...
- [ ] 2.7 Implement `identity` (design §13) as a digest over a canonical
...
- [ ] 2.8 Implement `limits` (design §9) with `crossing_tolerance` and
      `max_crossings` only, and a test asserting the other three running
      limits are absent.
- [ ] 2.9 RED then green: `clocked_of(root)` (design §12) on `program_of`'s
...
- [ ] 3.1 RED: assert directly that `'$own'` is not a name of the document's
...
- [ ] 3.2 Implement the minted own-name: `_own`, lengthened by a leading
...
- [ ] 3.3 Assert the shared expression parser and `bind_expressions` are
      UNCHANGED, by diff.

## 4. The document: version 8, the tables, the object (design §1-§5)

- [ ] 4.1 RED: `document_version` gives 8 for a clocked root — including a
      ...
      ladder, 2 through 8.
- [ ] 4.2 Implement `CLOCKED_DOCUMENT_VERSION = 8` and the eighth rung of
      `document_version` (`core/serializer.py:446-484`), clocked dominating.
- [ ] 4.3 Implement `states_table` over the `drivers_table` rules
      (`serializer.py:352-376`) — the same five fields, keys sorted — and a
      test that the two tables name disjoint id sets.
- [ ] 4.4 Implement `compiled_clocked(node)` and `clocked_block` in the
...
- [ ] 4.5 RED then green: `document_body(..., clocked=...)` publishes
...
- [ ] 4.6 Implement `_collect_clocked_slots` and extend `bind_document`'s id
      set with the states' keys, the clock and the own-name, and test that a
      minted `_b` name can collide with none of them.
- [ ] 4.7 Re-aim the gate (design §2): `_refuse_a_clocked_model` becomes the
...
- [ ] 4.8 Assert that a stateless tree still costs `document_body` exactly one
      structural walk that renders nothing.

## 5. The clock, `$t`, and `symbolic_document` (design §10, §11)

- [ ] 5.1 RED: a clocked root declaring `Time.elapsed()` whose part is posed
      from `self.time` publishes the free name `time`, and `clocked.clock`
      reads `"time"`.
- [ ] 5.2 Implement the one line in `symbolic_document`'s clocked branch —
...
- [ ] 5.3 RED then green: a clocked root with NO time base publishes the
      animation variable in that same operation, byte for byte as a root
      declaring no base at all, and `clocked.clock` is `null`.
- [ ] 5.4 Assert `animation` carries `fps` and `frames` and no `loop` under
      both clocked bases.
- [ ] 5.5 Confirm against the source and by test what design §11 measured:
...
- [ ] 5.6 Assert the control refusal is unchanged in both places
...
- [ ] 6.1 Record the probe as a TEST: a clocked root declaring an instruction
...
- [ ] 6.2 Rename `instructions_table`'s `running=` parameter to say what it
...
- [ ] 6.3 RED then green: a version 8 document publishes EVERY declared
      instruction, each with exactly one of `targets` and `by`, and a
      stateless untimed document still omits the relative ones.
- [ ] 6.4 Test that an instruction naming a STATE reaches NO document: the
...
- [ ] 7.1 `core/builder.py`: call `compiled_clocked` beside
      ...
      warning once.
- [ ] 7.2 `core/export.py`: the same, and test that the export is WRITTEN with
      the warning rather than refused.
- [ ] 7.3 `viewers/browser.py`: the same, publishing the two tables off the
...
- [ ] 7.4 Test that `--renderer openscad` on a clocked model renders the
      INITIAL BANK, that `--drive` poses a declared driver, and that `--drive`
      naming a state is refused by name.
- [ ] 7.5 `tests/test_viewer_bundle.py`: added tests only — a viewer reporting
      ...
      UNCHANGED, by diff.
- [ ] 7.6 Assert the four producers reach the re-aimed gate: each is tested
      with a clocked tree and no compiled machine and is refused.

## 8. The Curta-shaped fixture (design §17)

- [ ] 8.1 Write `tests/clocked_project/calculator.py`: four dials of one
...
- [ ] 8.2 Compute BY HAND the expected bank after a small scripted sequence —
...
- [ ] 8.3 Measure and record: the compile cost of this fixture, the node count
...
- [ ] 9.1 Write `tools/generate_clocked_corpus.py` on
      ...
      and `build`/`main`.
- [ ] 9.2 Implement the refusal recording — the kind and the qualified names,
      with the bank after each refusal asserted equal to the bank before it —
      and NOT the message text.
- [ ] 9.3 Implement `uncovered_features` over the full inventory in
      `design.md` §16 and the export spec, stated in the tool rather than
      inferred.
- [ ] 9.4 Script the machines: `counter`, `register`, `clearing`, `pawl`,
      ...
      existing fixture.
- [ ] 9.5 Write `tests/clocked-corpus.json` and record its size and machine
      count.
- [ ] 9.6 Write `tests/test_clocked_corpus.py`: the framework reproduces its
...
- [ ] 9.7 Test the generator's refusal directly, at least four ways — a
      ...
      the generator.
- [ ] 9.8 Implement the generator's EXACTNESS GUARD: refuse a machine whose
...
- [ ] 9.9 Record in `evidence.md` the basis of the exactness claim, operation
...
- [ ] 10.1 Pin the version 8 documents of `counter`, `register`, `pawl`,
      `pendulum` and `calculator` as committed fixture files, compared
      LITERALLY.
- [ ] 10.2 Write `tests/test_clocked_document.py` for everything not already
...
- [ ] 12.4 Write `evidence.md`: the red log, every measurement, every
      deviation reported as a deviation, and the open questions.
- [ ] 12.5 Extract ONE ADR, candidate **ADR-128** (NODE), after implementation
