...
**Amended:** 2026-09-17 by [ADR-129](./ADR-129-an-instruction-under-a-clocked-root-is-one-request.md) — section "Instructions are published in the version 5 shape, with no meaning": the SHAPE and the published bytes stand, and the MEANING moves. An instruction under a clocked root is ONE REQUEST over the one driver it names, `sim.trigger(name)` makes it and RETURNS it rather than being refused by name, and an instruction naming NONE or MORE THAN ONE driver is refused where this decision's machine is COMPILED — so every instruction a version 8 document carries is one a consumer can play, and the document itself gains no field, no key and no version.
**Depends on:**
