...
**Source code for machines.** The tree, shared parameters, mechanical
relations, inputs and retained state describe a whole machine. CAD
backends supply its geometry; assembly, operation and tests share its
...
server, the development page, the bundle routes and the reload and
...
auto-mounts `data-machinome-widget` containers, animates `$t` client-side (play/
pause + timeline when animated), and honors `?t=`/`?autoplay=0`. The browser
global is `MachinomeViewer`, exposing API 20 in the matching 0.2.0 viewer
source and accepting document schemas 1–8. That package remains unpublished.
Running models are executed through `run()` and clocked models through
`machine()`; a host can check both the API and readable schemas before mounting.
...
independent viewer (API 23 at the time, API 27 in 0.7.0) reads version 10 and executes these
