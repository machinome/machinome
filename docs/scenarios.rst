...
did the whole run look like. `machinome.simulation` provides three
execution models, selected by what the machine declares:
...
.. list-table::
   ...
   * - Machine
     - Construction
     - What advances
   * - Posed, with no stored state
     - ``Sim(machine, dt=0.02)``
     - Scheduled input ramps on fixed ticks; pose follows current inputs.
   * - Running, with ``Time.running()``
     - ``Sim(machine, dt=0.02)``
     - Coordinate history, movement and rates, crossings and stops.
   * - Clocked, with a ``State`` anywhere in the tree
     - ``Sim(machine)``
     - One input request at a time, with retained values committed at events.

``Time.elapsed()`` declares non-wrapping seconds without choosing running
mechanics. On a clocked machine it supplies a banked clock; without
``State`` the model stays stepped. Start with the stepped loop below,
then read the running and clocked sections when your mechanism needs them.
...
A running root's document starts at **version 5**; self-reading laws
require **6**, and selected relation blocks require **7**. Beside the
...
does not understand. ``machinome build``, ``machinome develop`` and ``machinome
...
browser, because a capture is a one-shot. The matching Machinome Viewer
0.2.0 source executes these programs and provides running controls;
that package is still awaiting publication. Check ``machinome viewer``
for the versions the installed viewer actually supports.
...
**Browser support.** Matching Machinome Viewer 0.2.0 source reads schema
8, operates clocked requests, displays stored state, draws instruction
transitions and advances an elapsed clock. It is not yet published.
When the installed viewer does not support 8, ``machinome build``,
``machinome develop`` and ``machinome export`` still publish and warn;
``machinome snapshot --renderer web`` refuses before starting the browser,
writes no image and never falls back to OpenSCAD. ``render()``, ``assemble()``,
...
* **Looping or running time.** A ``State`` under ``Time(loop=)`` is refused — a loop
...
* **The cadence surface.** ``run``, ``at``, ``every``,
  ...
  on that list: an instruction is one request, as described above.
...
* A port, joint coordinate or derived coordinate as a SOURCE; a
  ...
  published in the version 5 instruction shape, and played as described above.
