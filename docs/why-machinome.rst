...
from another. In Machinome every leaf part is written against the
...
Machinome is inspired by web-development culture: a dev server
...
invariants at every step. Machinome's test framework makes those
assertions ordinary test cases, run by its own runner or by pytest.

Open Source
-----------

Machinome is released under the Apache License 2.0. You are free to
