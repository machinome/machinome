...
`Build123dSheetNode`'s docstring gives: `solid_node.node` is on every
`solid` invocation's path and build123d costs about 1.6 s.
"""
...
def _is_marking(value):
    ...
    importing this module, so `solid_node/node/declarative.py` keeps
...
class Svg:
    ...
    def regions(self) -> list:
        ...
        about 1.6 s and `solid_node.node` is on every `solid`
