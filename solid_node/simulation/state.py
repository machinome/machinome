...
(`solid_node.motion.couplings.commit`) is the only thing that writes
...
`solid_node.node` imports nothing from here. A project that names no
...
from solid_node.node.qualified import StateDeclaration, declared_states_of
